...
export function drawAthleteEnemy(
  ...
  for (let ai = 4; ai >= 1; ai--) {
    ...
    const aiOffset = ai * size * 0.1 + Math.sin(runCycle + ai * 0.8) * size * 0.015;
    ...
    ctx.beginPath();
    ctx.ellipse(0, -size * 0.1, size * 0.12, size * 0.24, leanForward * 0.3, 0, TAU);
    ctx.fill();
  ...
  const motionGrad = ctx.createRadialGradient(cx - size * 0.15, y - size * 0.08 - bodyBob, size * 0.04, cx - size * 0.15, y - size * 0.08 - bodyBob, size * 0.6);
  ...
  ctx.beginPath();
  ctx.ellipse(cx - size * 0.2, y - size * 0.08 - bodyBob, size * 0.7, size * 0.28, 0, 0, TAU);
  ctx.fill();
  ...
  const capeGrad = ctx.createLinearGradient(cx, y - size * 0.28, cx + capeWave, y + size * 0.1);
  ...
  ctx.quadraticCurveTo(cx - size * 0.2 + capeWave, y + size * 0.0, cx - size * 0.22 + capeWave, y + size * 0.12 - bodyBob);
  ctx.lineTo(cx + size * 0.06 + capeWave * 0.5, y + size * 0.08 - bodyBob);
  ctx.quadraticCurveTo(cx + size * 0.04, y - size * 0.08 - bodyBob, cx + size * 0.14, y - size * 0.26 - bodyBob);
  ctx.closePath();
  ctx.fill();
  ...
  // Muscular torso (skin)
  const torsoGrad = ctx.createLinearGradient(-size * 0.2, -size * 0.22, size * 0.2, size * 0.16);
  ...
  ctx.bezierCurveTo(-size * 0.24, size * 0.04, -size * 0.22, -size * 0.1, -size * 0.18, -size * 0.22);
  ctx.bezierCurveTo(-size * 0.1, -size * 0.26, size * 0.1, -size * 0.26, size * 0.18, -size * 0.22);
  ctx.bezierCurveTo(size * 0.22, -size * 0.1, size * 0.24, size * 0.04, size * 0.2, size * 0.14);
  ctx.closePath();
  ctx.fill();
  ...
  for (const side of [-1, 1]) {
    ...
    const agGrad = ctx.createLinearGradient(agX - size * 0.03, agY, agX + size * 0.03, agY + size * 0.08);
    agGrad.addColorStop(0, bronzeLight);
    agGrad.addColorStop(0.5, bronze);
    agGrad.addColorStop(1, bronzeDark);
    ctx.fillStyle = agGrad;
    ctx.beginPath();
    ctx.ellipse(agX, agY + size * 0.01, size * 0.04, size * 0.035, side * 0.3, 0, TAU);
    ctx.fill();
  }

  // Leather waist guard (NOT armor skirt)
  ctx.fillStyle = bronzeDark;
  ctx.fillRect(cx - size * 0.16, y + size * 0.0 - bodyBob, size * 0.32, size * 0.025);
  ctx.fillStyle = bronzeLight;
  ctx.fillRect(cx - size * 0.02, y - size * 0.005 - bodyBob, size * 0.04, size * 0.035);

  // Arms (pumping while running)
  drawPathArm(ctx, cx - size * 0.24, y - size * 0.22 - bodyBob, size, time, zoom, -1, {
    color: skinTone,
    colorDark: skinDark,
    handColor: skinDark,
    trimColor: bodyColor,
  ...
  // Face
  const faceGrad = ctx.createRadialGradient(headX, headY + size * 0.02, 0, headX, headY, size * 0.14);
  ...
  for (const side of [-1, 1]) {
    ctx.fillStyle = "#fff";
    ctx.beginPath();
    ctx.ellipse(headX + side * size * 0.05, headY - size * 0.015, size * 0.022, size * 0.016, 0, 0, TAU);
    ctx.fill();
    ctx.fillStyle = bodyColor;
    ctx.beginPath();
    ctx.arc(headX + side * size * 0.05, headY - size * 0.015, size * 0.01, 0, TAU);
    ctx.fill();
    ctx.fillStyle = "#111";
    ctx.beginPath();
    ctx.arc(headX + side * size * 0.05, headY - size * 0.015, size * 0.005, 0, TAU);
    ctx.fill();
  }
  // War paint stripe across eyes
  ctx.fillStyle = bodyColorDark;
  ctx.globalAlpha = 0.5;
  ctx.fillRect(headX - size * 0.12, headY - size * 0.03, size * 0.24, size * 0.025);
  ctx.globalAlpha = 1;
  // Determined jaw
  ...
  // Bronze headband
  const hbGrad = ctx.createLinearGradient(headX - size * 0.14, headY - size * 0.06, headX + size * 0.14, headY - size * 0.06);
  ...
  ctx.quadraticCurveTo(headX, headY - size * 0.08, headX + size * 0.14, headY - size * 0.04);
  ctx.quadraticCurveTo(headX, headY - size * 0.05, headX - size * 0.14, headY - size * 0.04);
  ctx.fill();
  ...
  for (const ws of [-1, 1]) {
    ...
    ctx.bezierCurveTo(
      wingX + ws * size * 0.06, wingY - size * 0.02 + wingFlutter,
      wingX + ws * size * 0.12, wingY - size * 0.08 + wingFlutter,
      wingX + ws * size * 0.14, wingY - size * 0.15 + wingFlutter,
    );
    ctx.bezierCurveTo(
      wingX + ws * size * 0.1, wingY - size * 0.06 + wingFlutter,
      wingX + ws * size * 0.05, wingY + size * 0.0,
      wingX, wingY + size * 0.02,
    );
    ctx.fill();
    ctx.strokeStyle = bodyColorDark;
  ...
  ctx.quadraticCurveTo(headX, headY - size * 0.18, headX + size * 0.1, headY - size * 0.08);
  ctx.quadraticCurveTo(headX, headY - size * 0.12, headX - size * 0.1, headY - size * 0.08);
  ctx.fill();
  ...
  for (const eSide of [-1, 1]) {
    ctx.beginPath();
    ctx.arc(headX + eSide * size * 0.05, headY - size * 0.015, size * 0.008, 0, TAU);
    ctx.fill();
  }
  clearShadow(ctx);
  ...
  if (isAttacking) {
    ...
    for (let rb = 0; rb < 8; rb++) {
      ...
      ctx.lineWidth = 1.5 * zoom;
      ctx.beginPath();
      ctx.moveTo(cx + Math.cos(rbAngle) * rbInner, y + size * 0.1 + Math.sin(rbAngle) * rbInner * ISO_Y_RATIO);
      ctx.lineTo(cx + Math.cos(rbAngle) * rbOuter, y + size * 0.1 + Math.sin(rbAngle) * rbOuter * ISO_Y_RATIO);
      ctx.stroke();
    }
  }
}
...
export function drawTigerFanEnemy(
  ...
  for (let f = 0; f < 7; f++) {
    ...
    const fy = y + size * 0.02 + size * 0.08 + Math.sin(time * 3 + f * 1.4) * size * 0.01 + (f % 2) * size * 0.02;
  ...
  // Muscular torso base
  const torsoGrad = ctx.createLinearGradient(cx - size * 0.2, y - size * 0.3, cx + size * 0.2, y + size * 0.06);
  ...
  ctx.bezierCurveTo(cx - size * 0.26, y - size * 0.05 - bodyBob, cx - size * 0.24, y - size * 0.2 - bodyBob, cx - size * 0.18, y - size * 0.3 - bodyBob);
  ctx.bezierCurveTo(cx - size * 0.1, y - size * 0.34 - bodyBob, cx + size * 0.1, y - size * 0.34 - bodyBob, cx + size * 0.18, y - size * 0.3 - bodyBob);
  ctx.bezierCurveTo(cx + size * 0.24, y - size * 0.2 - bodyBob, cx + size * 0.26, y - size * 0.05 - bodyBob, cx + size * 0.22, y + size * 0.06 - bodyBob);
  ctx.closePath();
  ctx.fill();

  // Open-front fur vest over torso
  const vestGrad = ctx.createLinearGradient(cx - size * 0.25, y, cx + size * 0.25, y);
  ...
  for (const side of [-1, 1]) {
    ...
    ctx.quadraticCurveTo(baseX + side * size * 0.02, y - size * 0.12 - bodyBob, baseX, y - size * 0.04 - bodyBob);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(baseX + side * size * 0.04, y - size * 0.18 - bodyBob);
    ctx.quadraticCurveTo(baseX + side * size * 0.05, y - size * 0.1 - bodyBob, baseX + side * size * 0.03, y - size * 0.02 - bodyBob);
    ctx.stroke();
  }
  ...
  for (const side of [-1, 1]) {
    ...
    for (let sp = 0; sp < 2; sp++) {
      ...
      ctx.beginPath();
      ctx.moveTo(spX + Math.cos(sAngle) * size * 0.04, spY + Math.sin(sAngle) * size * 0.03);
      ctx.lineTo(spX + Math.cos(sAngle) * size * 0.08, spY + Math.sin(sAngle) * size * 0.04 - size * 0.02);
      ctx.lineTo(spX + Math.cos(sAngle) * size * 0.05, spY + Math.sin(sAngle) * size * 0.035);
      ctx.fill();
    }
  }
  ...
  ctx.fillRect(cx - size * 0.18, y + size * 0.0 - bodyBob, size * 0.36, size * 0.025);
  ctx.fillStyle = "#e8d8c0";
  ctx.beginPath();
  ...
  drawPathArm(ctx, cx - size * 0.26, y - size * 0.22 - bodyBob, size, time, zoom, -1, {
    color: leather,
    colorDark: leatherDark,
    handColor: "#c4a878",
    trimColor: bodyColor,
    ...
    width: 0.08,
    shoulderAngle: 0.3 + Math.sin(time * 3) * 0.04 + (isAttacking ? -attackIntensity * 0.25 : 0),
    ...
    onWeapon: (wCtx) => {
      ...
      const shW = size * 0.15;
      const shH = size * 0.22;
      const shGrad = wCtx.createLinearGradient(-shW, -shH * 0.3, shW, shH * 0.3);
      ...
      for (let toe = 0; toe < 3; toe++) {
        ...
        wCtx.beginPath();
        wCtx.arc(Math.cos(tAngle) * size * 0.028, Math.sin(tAngle) * size * 0.028 - size * 0.005, size * 0.009, 0, TAU);
        wCtx.fill();
      }
    },
  });

  // Right arm — war banner
  const bannerSwing = Math.sin(time * 4.5 * attackIntensity) * (isAttacking ? 0.08 : 0.05);
  drawPathArm(ctx, cx + size * 0.26, y - size * 0.22 - bodyBob, size, time, zoom, 1, {
    color: leather,
    colorDark: leatherDark,
    handColor: "#c4a878",
    ...
    onWeapon: (wCtx) => {
      ...
      const poleLen = size * 0.9;
      const pGrad = wCtx.createLinearGradient(-size * 0.015, 0, size * 0.015, 0);
      ...
      const bBotY = -poleLen + size * 0.48;
      const bW = size * 0.35;
      const wave1 = Math.sin(time * 4.5 * attackIntensity) * size * 0.04;
      const wave2 = Math.sin(time * 4.5 * attackIntensity + 1.5) * size * 0.05;
      ...
      wCtx.bezierCurveTo(bW * 0.3 + wave1 * 0.5, bTopY + (bBotY - bTopY) * 0.3, bW * 0.7 + wave1, bTopY + (bBotY - bTopY) * 0.5, bW + wave2, bTopY + (bBotY - bTopY) * 0.5);
      wCtx.bezierCurveTo(bW * 0.8 + wave2, bTopY + (bBotY - bTopY) * 0.75, bW * 0.4 + wave1, bBotY - size * 0.02, bannerSwing * size * 0.5, bBotY);
      wCtx.closePath();
      wCtx.fill();
      ...
      wCtx.fillStyle = "#fff";
      wCtx.font = `bold ${size * 0.07}px serif`;
      wCtx.textAlign = "center";
      wCtx.textBaseline = "middle";
      wCtx.fillText("GO", 0, -size * 0.06);
      wCtx.font = `bold ${size * 0.06}px serif`;
      wCtx.fillText("TIGERS", 0, size * 0.03);
      wCtx.restore();
      wCtx.strokeStyle = bodyColorLight;
      wCtx.lineWidth = 1.2 * zoom;
      for (let fr = 0; fr < 5; fr++) {
        const t = (fr + 0.5) / 5;
        const frX = bannerSwing * size * 0.5 * (1 - t) + (bW * 0.4 + wave1) * t;
  ...
  // HEAD — Tiger cowl/mask (NOT armored helm)
  const headX = cx;
  ...
  for (const side of [-1, 1]) {
    ...
    ctx.quadraticCurveTo(headX + side * size * 0.08, headY - size * 0.02, headX + side * size * 0.1, headY + size * 0.04);
    ctx.stroke();
    ctx.beginPath();
  ...
  const cowlGrad = ctx.createRadialGradient(headX, headY - size * 0.08, 0, headX, headY, size * 0.22);
  ...
  ctx.quadraticCurveTo(headX - size * 0.2, headY - size * 0.08, headX - size * 0.14, headY - size * 0.16);
  ctx.quadraticCurveTo(headX, headY - size * 0.24, headX + size * 0.14, headY - size * 0.16);
  ctx.quadraticCurveTo(headX + size * 0.2, headY - size * 0.08, headX + size * 0.16, headY + size * 0.04);
  ctx.quadraticCurveTo(headX + size * 0.1, headY - size * 0.02, headX + size * 0.08, headY - size * 0.06);
  ctx.quadraticCurveTo(headX, headY - size * 0.1, headX - size * 0.08, headY - size * 0.06);
  ctx.quadraticCurveTo(headX - size * 0.1, headY - size * 0.02, headX - size * 0.16, headY + size * 0.04);
  ctx.fill();
  ...
  for (const ps of [-1, 1]) {
    ...
    ctx.quadraticCurveTo(headX + ps * size * 0.1, headY - size * 0.12, headX + ps * size * 0.08, headY - size * 0.04);
    ctx.stroke();
    ctx.beginPath();
  ...
  if (isAttacking) {
    ...
    for (let sl = 0; sl < 6; sl++) {
      ...
      ctx.lineWidth = 1.5 * zoom;
      ctx.beginPath();
      ctx.moveTo(cx + Math.cos(slAngle) * slInner, y + size * 0.3 + Math.sin(slAngle) * slInner * ISO_Y_RATIO);
      ctx.lineTo(cx + Math.cos(slAngle) * slOuter, y + size * 0.3 + Math.sin(slAngle) * slOuter * ISO_Y_RATIO);
      ctx.stroke();
    }
    ctx.fillStyle = `rgba(249, 115, 22, ${slamForce * 0.15})`;
    ctx.beginPath();
    ctx.ellipse(cx, y + size * 0.1, size * 0.2 * slamForce, size * 0.25 * slamForce, 0, 0, TAU);
    ctx.fill();
  }
}
