...
export function renderTroop(
  ...
  const facingRight =
    troop.facingRight ??
    (typeof troop.rotation === "number" ? Math.cos(troop.rotation) >= 0 : true);
...
interface TroopMasterworkStyle {
  rimLight: string;
  aura: string;
  rune: string;
  metalSheen: string;
