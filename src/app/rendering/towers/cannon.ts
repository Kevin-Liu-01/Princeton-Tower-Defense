  ...
  const drawMuzzleSection = () => {
    ...
    if (level >= 3) {
      ...
      for (let f = 0; f < 4; f++) {
        ...
        ctx.lineTo(
          finEndPt2.x + fOuter.x * 0.8,
          finEndPt2.y + fOuter.y * 0.8,
        );
        ctx.lineTo(
  ...
  const drawNearCap = () => {
    ...
    if (facingFwd) {
      ...
      for (let i = 1; i < hexSides; i++)
        ctx.lineTo(
          mCapPt.x + riflingVerts[i].x,
          mCapPt.y + riflingVerts[i].y,
        );
      ctx.closePath();
      ctx.stroke();
    }
...
export function drawHeavyCannonBarrel(
  ...
  const drawNearCap = () => {
    ...
    if (facingFwd) {
      ...
      ctx.moveTo(
        mCapPt.x + riflingVerts2[0].x,
        mCapPt.y + riflingVerts2[0].y,
      );
      for (let i = 1; i < hexSides; i++)
        ctx.lineTo(
...
export function drawGatlingBarrels(
  ...
  const drawBarrels = () => {
    for (const bd of barrelData) {
      ...
      const c = Math.max(
        30,
        Math.min(140, Math.floor(45 + shade * 65 + tint)),
      );
