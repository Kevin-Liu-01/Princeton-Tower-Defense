...
export const ENEMY_COLORS: Record<string, string> = {
  ...
  skeleton_footman: "#d4c5a0",
  skeleton_knight: "#8b8068",
  skeleton_archer: "#c4b590",
  skeleton_king: "#daa520",
  zombie_shambler: "#5a7247",
  zombie_brute: "#4a5d3a",
  zombie_spitter: "#7cb342",
  ghoul: "#6b5b73",
  dark_knight: "#2d2d3d",
  death_knight: "#1a1a2e",
  fallen_paladin: "#4a3f6b",
  black_guard: "#1c1c2e",
  lich: "#4fc3f7",
  wraith: "#b39ddb",
  bone_mage: "#ce93d8",
  ...
  hellhound: "#ff6f00",
  doom_herald: "#311b92",
  titan_of_nassau: "#e65100",
  swamp_leviathan: "#1b5e20",
  sphinx_guardian: "#f9a825",
  frost_colossus: "#4dd0e1",
  inferno_wyrm: "#d50000",
  orb_weaver: "#4a3728",
  mantis: "#65a30d",
  bombardier_beetle: "#1e3a5f",
  mosquito: "#6b2142",
  centipede: "#7c2d12",
  dragonfly: "#0ea5e9",
  silk_moth: "#c4b5fd",
  ...
  const renderEnemy = useCallback((time: number) => {
    ...
    switch (type) {
      ...
      case "skeleton_footman": {
        // Bone-white skeleton soldier with rusty sword
        const boneColor = "#d4c5a0";
        const boneDark = "#b8a882";
        const baseY = cy + 8 * scale - bounce;

        // Legs
        ctx.strokeStyle = boneDark;
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY);
        ctx.lineTo(cx - 5 * scale, baseY + 10 * scale);
        ctx.moveTo(cx + 3 * scale, baseY);
        ctx.lineTo(cx + 5 * scale, baseY + 10 * scale);
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 4 * scale, 6 * scale, 8 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = boneDark;
        ctx.lineWidth = 1 * scale;
        for (let i = -2; i <= 2; i++) {
          ctx.beginPath();
          ctx.moveTo(cx - 4 * scale, baseY - 4 * scale + i * 2 * scale);
          ctx.lineTo(cx + 4 * scale, baseY - 4 * scale + i * 2 * scale);
          ctx.stroke();
        }
        ...
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 6 * scale, baseY - 8 * scale);
        ctx.lineTo(cx - 10 * scale, baseY + 2 * scale);
        ctx.moveTo(cx + 6 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 10 * scale, baseY - 4 * scale);
        ctx.stroke();

        // Skull
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale, 6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.ellipse(cx - 2 * scale, baseY - 15 * scale, 1.5 * scale, 2 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 2 * scale, baseY - 15 * scale, 1.5 * scale, 2 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.moveTo(cx - 1.5 * scale, baseY - 12 * scale);
        ctx.lineTo(cx, baseY - 11 * scale);
        ctx.lineTo(cx + 1.5 * scale, baseY - 12 * scale);
        ctx.closePath();
        ctx.fill();
        // Jaw
        ctx.strokeStyle = boneDark;
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY - 10 * scale);
        ctx.lineTo(cx, baseY - 9 * scale);
        ctx.lineTo(cx + 3 * scale, baseY - 10 * scale);
        ctx.stroke();

        // Rusty sword in right hand
        ctx.fillStyle = "#8b6914";
        ctx.fillRect(cx + 9 * scale, baseY - 12 * scale, 2 * scale, 4 * scale);
        ctx.fillStyle = "#a0a0a0";
        ctx.fillRect(cx + 9.5 * scale, baseY - 20 * scale, 1.5 * scale, 9 * scale);

        break;
      }
      case "skeleton_knight": {
        // Armored skeleton with iron-gray plates, red eye slits, shield
        const armorColor = "#8b8068";
        const armorDark = "#5a5347";
        const baseY = cy + 8 * scale - bounce;

        // Legs with greaves
        ctx.fillStyle = armorDark;
        ctx.fillRect(cx - 5 * scale, baseY + 2 * scale, 3 * scale, 9 * scale);
        ctx.fillRect(cx + 2 * scale, baseY + 2 * scale, 3 * scale, 9 * scale);

        // Armored torso
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY - 2 * scale);
        ctx.lineTo(cx - 7 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 7 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 8 * scale, baseY - 2 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.ellipse(cx - 8 * scale, baseY - 11 * scale, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(cx + 8 * scale, baseY - 11 * scale, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale, 6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = armorDark;
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale, 6 * scale, Math.PI * 0.8, Math.PI * 2.2);
        ctx.fill();

        // Red eye slits
        ctx.fillStyle = "#ff2222";
        ctx.fillRect(cx - 3.5 * scale, baseY - 17 * scale, 2.5 * scale, 1.2 * scale);
        ctx.fillRect(cx + 1 * scale, baseY - 17 * scale, 2.5 * scale, 1.2 * scale);
        if (animated) {
          ctx.fillStyle = `rgba(255, 50, 50, ${0.3 + Math.sin(t * 2) * 0.2})`;
          ctx.beginPath();
          ctx.arc(cx - 2 * scale, baseY - 16.5 * scale, 3 * scale, 0, Math.PI * 2);
          ctx.arc(cx + 2 * scale, baseY - 16.5 * scale, 3 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.fillStyle = armorDark;
        ctx.beginPath();
        ctx.moveTo(cx - 12 * scale, baseY - 10 * scale);
        ctx.lineTo(cx - 15 * scale, baseY - 4 * scale);
        ctx.lineTo(cx - 12 * scale, baseY + 4 * scale);
        ctx.lineTo(cx - 9 * scale, baseY - 4 * scale);
        ctx.closePath();
        ctx.fill();
      ...
      case "skeleton_archer": {
        // Lean skeleton with drawn bow, green eye glow
        const boneColor = "#c4b590";
        const boneDark = "#a09070";
        const baseY = cy + 8 * scale - bounce;
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY);
        ctx.lineTo(cx - 4 * scale, baseY + 10 * scale);
        ctx.moveTo(cx + 3 * scale, baseY);
        ctx.lineTo(cx + 5 * scale, baseY + 10 * scale);
        ctx.stroke();
        ...
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 4 * scale, 5 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = boneDark;
        ctx.lineWidth = 0.8 * scale;
        for (let i = -2; i <= 1; i++) {
          ctx.beginPath();
          ctx.moveTo(cx - 3.5 * scale, baseY - 4 * scale + i * 2 * scale);
          ctx.lineTo(cx + 3.5 * scale, baseY - 4 * scale + i * 2 * scale);
          ctx.stroke();
        }
        ...
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 13 * scale, 5.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Green glowing eyes
        const eyeGlow = animated ? 0.6 + Math.sin(t * 3) * 0.3 : 0.7;
        ctx.fillStyle = `rgba(76, 175, 80, ${eyeGlow})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 14 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 14 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(76, 255, 80, ${eyeGlow * 0.3})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 14 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 14 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.arc(cx - 10 * scale, baseY - 6 * scale, 10 * scale, -Math.PI * 0.4, Math.PI * 0.4);
        ctx.stroke();
        // Bowstring
        ctx.strokeStyle = "#ccc";
        ctx.lineWidth = 0.5 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 10 * scale + 10 * scale * Math.cos(-Math.PI * 0.4), baseY - 6 * scale + 10 * scale * Math.sin(-Math.PI * 0.4));
        ctx.lineTo(cx - 4 * scale, baseY - 6 * scale);
        ctx.lineTo(cx - 10 * scale + 10 * scale * Math.cos(Math.PI * 0.4), baseY - 6 * scale + 10 * scale * Math.sin(Math.PI * 0.4));
        ctx.stroke();

        // Arrow
        ctx.strokeStyle = "#8d6e32";
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 4 * scale, baseY - 6 * scale);
        ctx.lineTo(cx + 10 * scale, baseY - 6 * scale);
        ctx.stroke();
        ctx.fillStyle = "#aaa";
        ctx.beginPath();
        ctx.moveTo(cx + 10 * scale, baseY - 6 * scale);
        ctx.lineTo(cx + 12 * scale, baseY - 7 * scale);
        ctx.lineTo(cx + 12 * scale, baseY - 5 * scale);
        ctx.closePath();
        ctx.fill();
      ...
      case "skeleton_king": {
        // BOSS: Golden crown, purple robe, scepter, purple eye flames
        const boneColor = "#d4c5a0";
        const robeColor = "#4a1a6b";
        const crownColor = "#daa520";
        const baseY = cy + 6 * scale - bounce;

        // Purple robe (wide bottom)
        ...
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = "#6a2a8b";
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx, baseY - 6 * scale);
        ctx.lineTo(cx, baseY + 12 * scale);
        ctx.stroke();

        // Bone shoulders
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.ellipse(cx - 8 * scale, baseY - 8 * scale, 3.5 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 8 * scale, baseY - 8 * scale, 3.5 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Purple eye flames
        const flamePulse = animated ? 0.6 + Math.sin(t * 4) * 0.4 : 0.8;
        ctx.fillStyle = `rgba(130, 50, 200, ${flamePulse})`;
        ctx.beginPath();
        ctx.ellipse(cx - 2.5 * scale, baseY - 15 * scale, 2 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 2.5 * scale, baseY - 15 * scale, 2 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        if (animated) {
          ctx.fillStyle = `rgba(180, 80, 255, ${flamePulse * 0.4})`;
          ctx.beginPath();
          ctx.ellipse(cx - 2.5 * scale, baseY - 17 * scale, 1.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
          ctx.ellipse(cx + 2.5 * scale, baseY - 17 * scale, 1.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.moveTo(cx - 1 * scale, baseY - 12.5 * scale);
        ctx.lineTo(cx, baseY - 11.5 * scale);
        ctx.lineTo(cx + 1 * scale, baseY - 12.5 * scale);
        ctx.closePath();
        ctx.fill();

        // Golden crown
        ctx.fillStyle = crownColor;
        ctx.fillRect(cx - 6 * scale, baseY - 20 * scale, 12 * scale, 3 * scale);
        ctx.beginPath();
        ctx.moveTo(cx - 5 * scale, baseY - 20 * scale);
        ctx.lineTo(cx - 4 * scale, baseY - 24 * scale);
        ctx.lineTo(cx - 2 * scale, baseY - 21 * scale);
        ctx.lineTo(cx, baseY - 25 * scale);
        ctx.lineTo(cx + 2 * scale, baseY - 21 * scale);
        ctx.lineTo(cx + 4 * scale, baseY - 24 * scale);
        ctx.lineTo(cx + 5 * scale, baseY - 20 * scale);
        ctx.closePath();
        ctx.fill();
        // Crown gems
        ctx.fillStyle = "#e040fb";
        ctx.beginPath();
        ctx.arc(cx, baseY - 23.5 * scale, 1 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Scepter (right hand)
        ctx.fillStyle = "#6d4c21";
        ctx.fillRect(cx + 12 * scale, baseY - 18 * scale, 2 * scale, 20 * scale);
        ctx.fillStyle = `rgba(180, 80, 255, ${flamePulse})`;
        ctx.beginPath();
        ctx.arc(cx + 13 * scale, baseY - 20 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(255, 255, 255, ${flamePulse * 0.6})`;
        ctx.beginPath();
        ctx.arc(cx + 13 * scale, baseY - 20 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        break;
      }
      case "zombie_shambler": {
        // Green-gray zombie, hunched, torn clothes, lopsided head
        const skinColor = "#5a7247";
        const skinDark = "#3e5030";
        const clothColor = "#3d3d2e";
        const baseY = cy + 8 * scale - bounce;

        // Legs (uneven)
        ctx.fillStyle = skinDark;
        ctx.fillRect(cx - 5 * scale, baseY + 2 * scale, 3.5 * scale, 9 * scale);
        ctx.fillRect(cx + 1 * scale, baseY + 3 * scale, 3.5 * scale, 8 * scale);

        // Hunched body
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx + 1 * scale, baseY - 2 * scale, 7 * scale, 9 * scale, 0.15, 0, Math.PI * 2);
        ctx.fill();

        // Torn shirt
        ctx.fillStyle = clothColor;
        ctx.beginPath();
        ctx.moveTo(cx - 5 * scale, baseY - 6 * scale);
        ctx.lineTo(cx + 6 * scale, baseY - 7 * scale);
        ctx.lineTo(cx + 7 * scale, baseY + 3 * scale);
        ctx.lineTo(cx - 2 * scale, baseY + 4 * scale);
        ctx.closePath();
        ctx.fill();

        // Arms (different lengths - zombie asymmetry)
        ctx.fillStyle = skinDark;
        ctx.save();
        ctx.translate(cx - 7 * scale, baseY - 6 * scale);
        ctx.rotate(0.3 + (animated ? Math.sin(t * 1.5) * 0.1 : 0));
        ctx.fillRect(-1.5 * scale, 0, 3 * scale, 10 * scale);
        ctx.restore();
        ctx.save();
        ctx.translate(cx + 7 * scale, baseY - 5 * scale);
        ctx.rotate(-0.2 + (animated ? Math.sin(t * 1.2) * 0.1 : 0));
        ...
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx + 2 * scale, baseY - 12 * scale, 5.5 * scale, 5 * scale, 0.2, 0, Math.PI * 2);
        ctx.fill();

        // Hollow eyes
        ctx.fillStyle = "#1a1a0a";
        ctx.beginPath();
        ctx.arc(cx, baseY - 13 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 4 * scale, baseY - 12.5 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 1 * scale, baseY - 9 * scale);
        ctx.quadraticCurveTo(cx + 2 * scale, baseY - 8 * scale, cx + 4 * scale, baseY - 9.5 * scale);
        ctx.stroke();

        break;
      }
      case "zombie_brute": {
        // Large armored zombie, bloated, chains, massive arms
        const skinColor = "#4a5d3a";
        const skinDark = "#2e3d20";
        const chainColor = "#808080";
        const baseY = cy + 6 * scale - bounce;

        // Thick legs
        ctx.fillStyle = skinDark;
        ctx.fillRect(cx - 6 * scale, baseY + 4 * scale, 5 * scale, 8 * scale);
        ctx.fillRect(cx + 1 * scale, baseY + 4 * scale, 5 * scale, 8 * scale);

        // Bloated body
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 2 * scale, 10 * scale, 11 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY - 10 * scale);
        ctx.lineTo(cx + 2 * scale, baseY + 6 * scale);
        ctx.stroke();
        for (let i = 0; i < 5; i++) {
          const py = baseY - 8 * scale + i * 3 * scale;
          const px = cx - 2.5 * scale + i * 1 * scale;
          ctx.beginPath();
          ctx.moveTo(px - 1.5 * scale, py - 1 * scale);
          ctx.lineTo(px + 1.5 * scale, py + 1 * scale);
          ctx.stroke();
        }
        ...
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY - 6 * scale);
        ctx.lineTo(cx + 8 * scale, baseY + 2 * scale);
        ctx.stroke();
        ...
        ctx.fillStyle = skinColor;
        ctx.save();
        ctx.translate(cx - 10 * scale, baseY - 6 * scale);
        ctx.rotate(0.2);
        ctx.fillRect(-3 * scale, 0, 5 * scale, 14 * scale);
        ctx.restore();
        ctx.save();
        ctx.translate(cx + 10 * scale, baseY - 6 * scale);
        ctx.rotate(-0.2);
        ctx.fillRect(-2 * scale, 0, 5 * scale, 14 * scale);
        ctx.restore();

        // Small head on big body
        ctx.fillStyle = skinDark;
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#ff4444";
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale, 1 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale, 1 * scale, 0, Math.PI * 2);
        ctx.fill();
      ...
      case "zombie_spitter": {
        // Ranged zombie with distended jaw, green bile dripping
        const skinColor = "#5a7247";
        const skinDark = "#3e5030";
        const bileColor = "#7cb342";
        const baseY = cy + 8 * scale - bounce;

        // Legs
        ctx.fillStyle = skinDark;
        ctx.fillRect(cx - 4 * scale, baseY + 2 * scale, 3 * scale, 8 * scale);
        ctx.fillRect(cx + 1 * scale, baseY + 2 * scale, 3 * scale, 8 * scale);

        // Hunched body
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 2 * scale, 7 * scale, 9 * scale, 0.1, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 13 * scale, 6 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Wide open distended jaw
        ctx.fillStyle = "#2a1a10";
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 9 * scale, 4 * scale, 3.5 * scale, 0, 0, Math.PI);
        ctx.fill();

        // Bile drip
        const dripPhase = animated ? (t * 2) % 1 : 0.5;
        ctx.fillStyle = `rgba(124, 179, 66, ${0.8 - dripPhase * 0.5})`;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 6 * scale + dripPhase * 8 * scale, 2 * scale, 1.5 * scale + dripPhase * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = bileColor;
        ctx.beginPath();
        ctx.arc(cx - 1 * scale, baseY - 8 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 1 * scale, baseY - 7.5 * scale, 1 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Glowing yellow-green eyes
        ctx.fillStyle = `rgba(200, 230, 50, ${animated ? 0.7 + Math.sin(t * 3) * 0.3 : 0.8})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 14 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 14 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
      ...
      case "ghoul": {
        // Fast feral creature, low crouching, long claws, wild eyes
        const skinColor = "#6b5b73";
        const skinDark = "#4a3d50";
        const baseY = cy + 10 * scale - bounce;

        // Low crouching body
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 4 * scale, 8 * scale, 6 * scale, 0.15, 0, Math.PI * 2);
        ctx.fill();

        // Bent legs
        ctx.fillStyle = skinDark;
        ctx.save();
        ctx.translate(cx - 5 * scale, baseY);
        ctx.rotate(-0.3);
        ...
        ctx.strokeStyle = skinDark;
        ctx.lineWidth = 2 * scale;
        const clawSwing = animated ? Math.sin(t * 4) * 0.15 : 0;
        ctx.save();
        ctx.translate(cx - 8 * scale, baseY - 6 * scale);
        ...
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 10 * scale, 5 * scale, 4.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Wild glowing eyes
        const eyeIntensity = animated ? 0.7 + Math.sin(t * 5) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(255, 200, 50, ${eyeIntensity})`;
        ctx.beginPath();
        ctx.ellipse(cx - 2 * scale, baseY - 11 * scale, 2 * scale, 1.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.ellipse(cx + 2 * scale, baseY - 11 * scale, 2 * scale, 1.5 * scale, 0.2, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 3 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 2 * scale, baseY - 6.5 * scale);
        ctx.lineTo(cx - 2 * scale, baseY - 6.5 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = "#ddd";
        ctx.beginPath();
        ctx.moveTo(cx - 1.5 * scale, baseY - 8 * scale);
        ctx.lineTo(cx - 1 * scale, baseY - 6.5 * scale);
        ctx.lineTo(cx - 0.5 * scale, baseY - 8 * scale);
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(cx + 0.5 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 1 * scale, baseY - 6.5 * scale);
        ctx.lineTo(cx + 1.5 * scale, baseY - 8 * scale);
        ctx.fill();

        break;
      }
      case "dark_knight": {
        // Heavy obsidian plate armor, red visor glow, cape, greatsword
        const armorColor = "#2d2d3d";
        const armorHighlight = "#3d3d55";
        const baseY = cy + 6 * scale - bounce;
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 7 * scale, baseY - 10 * scale);
        ctx.quadraticCurveTo(cx - 10 * scale, baseY + 5 * scale, cx - 8 * scale, baseY + 12 * scale);
        ctx.lineTo(cx + 8 * scale, baseY + 12 * scale);
        ctx.quadraticCurveTo(cx + 10 * scale, baseY + 5 * scale, cx + 7 * scale, baseY - 10 * scale);
        ctx.closePath();
        ctx.fill();

        // Legs
        ctx.fillStyle = armorColor;
        ctx.fillRect(cx - 5 * scale, baseY + 2 * scale, 4 * scale, 9 * scale);
        ctx.fillRect(cx + 1 * scale, baseY + 2 * scale, 4 * scale, 9 * scale);

        // Heavy torso
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.moveTo(cx - 9 * scale, baseY);
        ctx.lineTo(cx - 8 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 8 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 9 * scale, baseY);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(cx - 10 * scale, baseY - 11 * scale, 5 * scale, 3.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(cx + 10 * scale, baseY - 11 * scale, 5 * scale, 3.5 * scale, 0.2, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale, 6.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = armorHighlight;
        ctx.beginPath();
        ctx.moveTo(cx, baseY - 22 * scale);
        ctx.lineTo(cx - 2 * scale, baseY - 17 * scale);
        ctx.lineTo(cx + 2 * scale, baseY - 17 * scale);
        ctx.closePath();
        ctx.fill();

        // Red visor glow
        const visorGlow = animated ? 0.7 + Math.sin(t * 2.5) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(200, 30, 30, ${visorGlow})`;
        ctx.fillRect(cx - 4 * scale, baseY - 17 * scale, 8 * scale, 2 * scale);
        ctx.fillStyle = `rgba(255, 50, 50, ${visorGlow * 0.3})`;
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale, 6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = "#333";
        ctx.fillRect(cx + 12 * scale, baseY - 24 * scale, 2 * scale, 28 * scale);
        ctx.fillStyle = armorHighlight;
        ctx.fillRect(cx + 9 * scale, baseY - 1 * scale, 8 * scale, 2 * scale);

        break;
      }
      case "death_knight": {
        // BOSS: Skull-faced helm, flaming sword, dark aura, massive pauldrons
        const armorColor = "#1a1a2e";
        const armorLight = "#2d2d44";
        const flameColor = "#fb923c";
        const baseY = cy + 6 * scale - bounce;

        // Dark aura
        if (animated) {
          const auraAlpha = 0.15 + Math.sin(t * 2) * 0.08;
          ctx.fillStyle = `rgba(26, 26, 46, ${auraAlpha})`;
          ctx.beginPath();
          ctx.arc(cx, baseY - 6 * scale, 20 * scale + Math.sin(t * 3) * 2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Cape with tattered edge
        ctx.fillStyle = "#0d0d1a";
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY - 10 * scale);
        ctx.quadraticCurveTo(cx - 12 * scale, baseY + 6 * scale, cx - 10 * scale, baseY + 14 * scale);
        ctx.lineTo(cx - 6 * scale, baseY + 12 * scale);
        ctx.lineTo(cx - 2 * scale, baseY + 14 * scale);
        ctx.lineTo(cx + 2 * scale, baseY + 12 * scale);
        ctx.lineTo(cx + 6 * scale, baseY + 14 * scale);
        ctx.lineTo(cx + 10 * scale, baseY + 12 * scale);
        ctx.quadraticCurveTo(cx + 12 * scale, baseY + 6 * scale, cx + 8 * scale, baseY - 10 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 10 * scale, baseY + 2 * scale);
        ctx.lineTo(cx - 9 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 9 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 10 * scale, baseY + 2 * scale);
        ctx.closePath();
        ctx.fill();

        // Massive pauldrons with skull trophies
        ctx.fillStyle = armorLight;
        ctx.beginPath();
        ctx.ellipse(cx - 12 * scale, baseY - 11 * scale, 6 * scale, 4 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(cx + 12 * scale, baseY - 11 * scale, 6 * scale, 4 * scale, 0.2, 0, Math.PI * 2);
        ctx.fill();
        // Trophy skulls on pauldrons
        ctx.fillStyle = "#d4c5a0";
        ctx.beginPath();
        ctx.arc(cx - 12 * scale, baseY - 12 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(cx + 12 * scale, baseY - 12 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.arc(cx - 13 * scale, baseY - 12.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(cx - 11 * scale, baseY - 12.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 11 * scale, baseY - 12.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 13 * scale, baseY - 12.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale, 6.5 * scale, Math.PI * 0.85, Math.PI * 2.15);
        ctx.fill();

        // Burning orange eyes
        const eyeBurn = animated ? 0.7 + Math.sin(t * 4) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(251, 146, 60, ${eyeBurn})`;
        ctx.beginPath();
        ctx.ellipse(cx - 2.5 * scale, baseY - 17 * scale, 2 * scale, 1.5 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 2.5 * scale, baseY - 17 * scale, 2 * scale, 1.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Flaming sword
        ctx.save();
        ctx.translate(cx + 14 * scale, baseY);
        ctx.rotate(-0.15);
        ctx.fillStyle = "#444";
        ctx.fillRect(-1 * scale, -3 * scale, 2 * scale, 6 * scale);
        ctx.fillStyle = "#888";
        ctx.fillRect(-1.5 * scale, -22 * scale, 3 * scale, 20 * scale);
        ctx.fillStyle = "#666";
        ctx.fillRect(-4 * scale, -4 * scale, 8 * scale, 2 * scale);
        if (animated) {
          const sFlame = 0.5 + Math.sin(t * 6) * 0.3;
          ctx.fillStyle = `rgba(251, 146, 60, ${sFlame})`;
          ctx.beginPath();
          ctx.moveTo(-2 * scale, -22 * scale);
          ctx.quadraticCurveTo(-3 * scale + Math.sin(t * 8) * scale, -28 * scale, 0, -30 * scale + Math.sin(t * 7) * 2 * scale);
          ctx.quadraticCurveTo(3 * scale - Math.sin(t * 8) * scale, -28 * scale, 2 * scale, -22 * scale);
          ctx.closePath();
          ctx.fill();
          ctx.fillStyle = `rgba(255, 220, 100, ${sFlame * 0.6})`;
          ctx.beginPath();
          ctx.arc(0, -25 * scale, 2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.restore();
      ...
      case "fallen_paladin": {
        // Corrupted knight, cracked white armor with purple veins, broken halo
        const armorColor = "#c0b8d0";
        const corruptColor = "#4a3f6b";
        const baseY = cy + 6 * scale - bounce;
        ...
        ctx.fillRect(cx - 5 * scale, baseY + 2 * scale, 4 * scale, 9 * scale);
        ctx.fillRect(cx + 1 * scale, baseY + 2 * scale, 4 * scale, 9 * scale);
        // Corruption veins on legs
        ctx.strokeStyle = corruptColor;
        ctx.lineWidth = 0.8 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 3 * scale, baseY + 3 * scale);
        ctx.lineTo(cx - 4 * scale, baseY + 9 * scale);
        ctx.moveTo(cx + 3 * scale, baseY + 3 * scale);
        ctx.lineTo(cx + 2 * scale, baseY + 9 * scale);
        ctx.stroke();

        // Torso
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY);
        ctx.lineTo(cx - 7 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 7 * scale, baseY - 12 * scale);
        ...
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 2 * scale, baseY - 10 * scale);
        ctx.lineTo(cx - 5 * scale, baseY - 4 * scale);
        ctx.lineTo(cx - 3 * scale, baseY);
        ctx.moveTo(cx + 1 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 4 * scale, baseY - 2 * scale);
        ctx.stroke();

        // Purple glow from cracks
        if (animated) {
          ctx.fillStyle = `rgba(74, 63, 107, ${0.2 + Math.sin(t * 2) * 0.1})`;
          ctx.beginPath();
          ctx.ellipse(cx, baseY - 5 * scale, 8 * scale, 10 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        // Visor
        ctx.fillStyle = corruptColor;
        ctx.fillRect(cx - 4 * scale, baseY - 17 * scale, 8 * scale, 2 * scale);

        // Broken halo
        ctx.strokeStyle = `rgba(255, 215, 0, ${animated ? 0.4 + Math.sin(t * 2) * 0.2 : 0.5})`;
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.arc(cx, baseY - 22 * scale, 5 * scale, Math.PI * 0.2, Math.PI * 1.3);
        ctx.stroke();
        ctx.strokeStyle = `rgba(100, 60, 180, ${animated ? 0.5 + Math.sin(t * 3) * 0.2 : 0.5})`;
        ctx.beginPath();
        ctx.arc(cx, baseY - 22 * scale, 5 * scale, Math.PI * 1.5, Math.PI * 2.1);
        ctx.stroke();

        // Weapon
        ctx.fillStyle = "#888";
        ctx.fillRect(cx + 10 * scale, baseY - 20 * scale, 2 * scale, 24 * scale);
        ctx.fillStyle = corruptColor;
        ctx.fillRect(cx + 7 * scale, baseY - 2 * scale, 8 * scale, 2 * scale);

        break;
      }
      case "black_guard": {
        // Tank with tower shield, midnight armor, enclosed helm
        const armorColor = "#1c1c2e";
        const armorLight = "#2a2a40";
        const baseY = cy + 6 * scale - bounce;
        ...
        ctx.fillStyle = armorColor;
        ctx.beginPath();
        ctx.moveTo(cx - 9 * scale, baseY + 2 * scale);
        ctx.lineTo(cx - 8 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 8 * scale, baseY - 12 * scale);
        ctx.lineTo(cx + 9 * scale, baseY + 2 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = armorLight;
        ctx.lineWidth = 0.8 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 6 * scale, baseY - 10 * scale);
        ctx.lineTo(cx + 6 * scale, baseY - 10 * scale);
        ctx.moveTo(cx - 7 * scale, baseY - 6 * scale);
        ctx.lineTo(cx + 7 * scale, baseY - 6 * scale);
        ctx.stroke();
        ...
        ctx.fillStyle = armorLight;
        ctx.beginPath();
        ctx.ellipse(cx - 9 * scale, baseY - 11 * scale, 4.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 9 * scale, baseY - 11 * scale, 4.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(cx, baseY - 22 * scale);
        ctx.lineTo(cx - 1.5 * scale, baseY - 17 * scale);
        ctx.lineTo(cx + 1.5 * scale, baseY - 17 * scale);
        ctx.closePath();
        ctx.fill();
        // Thin visor slit
        ctx.fillStyle = `rgba(100, 100, 140, ${animated ? 0.5 + Math.sin(t * 2) * 0.2 : 0.6})`;
        ctx.fillRect(cx - 4 * scale, baseY - 17 * scale, 8 * scale, 1.5 * scale);

        // TOWER SHIELD (left side, massive)
        ctx.fillStyle = armorLight;
        ctx.beginPath();
        ctx.moveTo(cx - 16 * scale, baseY - 14 * scale);
        ctx.lineTo(cx - 10 * scale, baseY - 16 * scale);
        ctx.lineTo(cx - 10 * scale, baseY + 8 * scale);
        ctx.lineTo(cx - 13 * scale, baseY + 10 * scale);
        ctx.lineTo(cx - 16 * scale, baseY + 8 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = armorColor;
        ctx.lineWidth = 1 * scale;
        ...
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 13 * scale, baseY - 10 * scale);
        ctx.lineTo(cx - 13 * scale, baseY + 4 * scale);
        ctx.moveTo(cx - 16 * scale, baseY - 3 * scale);
        ctx.lineTo(cx - 10 * scale, baseY - 3 * scale);
        ctx.stroke();

        break;
      }
      case "lich": {
        // BOSS undead mage: skeletal in grand robes, floating gem, icy blue eyes
        const robeColor = "#1a1a3e";
        const boneColor = "#d4c5a0";
        const icyBlue = "#4fc3f7";
        const baseY = cy + 6 * scale - bounce;

        // Magical aura
        if (animated) {
          ctx.fillStyle = `rgba(79, 195, 247, ${0.08 + Math.sin(t * 2) * 0.04})`;
          ctx.beginPath();
          ctx.arc(cx, baseY - 6 * scale, 22 * scale + Math.sin(t * 3) * 2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Grand robes (wide, flowing)
        ctx.fillStyle = robeColor;
        ctx.beginPath();
        ctx.moveTo(cx - 12 * scale, baseY + 14 * scale);
        ctx.quadraticCurveTo(cx - 10 * scale, baseY + 2 * scale, cx - 7 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 7 * scale, baseY - 8 * scale);
        ctx.quadraticCurveTo(cx + 10 * scale, baseY + 2 * scale, cx + 12 * scale, baseY + 14 * scale);
        ctx.closePath();
        ctx.fill();
        // Robe trim
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 12 * scale, baseY + 14 * scale);
        ctx.lineTo(cx + 12 * scale, baseY + 14 * scale);
        ctx.stroke();

        // Bone hands
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx - 10 * scale, baseY - 2 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 10 * scale, baseY - 2 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.fillStyle = robeColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale, 7 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale, 5 * scale, 0, Math.PI * 2);
        ...
        // Icy blue eyes
        const eyeGlow = animated ? 0.7 + Math.sin(t * 3) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(79, 195, 247, ${eyeGlow})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale, 1.8 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale, 1.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(79, 195, 247, ${eyeGlow * 0.3})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale, 4 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale, 4 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Staff (right side)
        ctx.fillStyle = "#4a3628";
        ctx.fillRect(cx + 13 * scale, baseY - 22 * scale, 2 * scale, 30 * scale);

        // Floating gem at staff top
        const gemPhase = animated ? Math.sin(t * 4) * 2 * scale : 0;
        ctx.fillStyle = `rgba(79, 195, 247, ${eyeGlow})`;
        ctx.beginPath();
        ctx.moveTo(cx + 14 * scale, baseY - 26 * scale + gemPhase);
        ctx.lineTo(cx + 11 * scale, baseY - 23 * scale + gemPhase);
        ctx.lineTo(cx + 14 * scale, baseY - 20 * scale + gemPhase);
        ctx.lineTo(cx + 17 * scale, baseY - 23 * scale + gemPhase);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = `rgba(255, 255, 255, ${eyeGlow * 0.5})`;
        ctx.beginPath();
        ctx.arc(cx + 14 * scale, baseY - 23 * scale + gemPhase, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        break;
      }
      case "wraith": {
        // Ethereal spirit, semi-transparent, no legs, chains, purple eyes
        const baseY = cy + 4 * scale - bounce;
        const floatOff = animated ? Math.sin(t * 2) * 3 * scale : 0;

        // Ghostly trail below (no legs)
        ctx.fillStyle = `rgba(179, 157, 219, 0.15)`;
        ctx.beginPath();
        ctx.moveTo(cx - 6 * scale, baseY + 4 * scale + floatOff);
        ctx.quadraticCurveTo(cx - 8 * scale, baseY + 14 * scale + floatOff, cx - 4 * scale, baseY + 18 * scale + floatOff);
        ctx.lineTo(cx + 4 * scale, baseY + 18 * scale + floatOff);
        ctx.quadraticCurveTo(cx + 8 * scale, baseY + 14 * scale + floatOff, cx + 6 * scale, baseY + 4 * scale + floatOff);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 2 * scale + floatOff, 7 * scale, 9 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(179, 157, 219, 0.5)`;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 2 * scale + floatOff, 5 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY - 4 * scale + floatOff);
        ctx.lineTo(cx - 12 * scale, baseY + 6 * scale + floatOff);
        ctx.moveTo(cx + 8 * scale, baseY - 4 * scale + floatOff);
        ctx.lineTo(cx + 12 * scale, baseY + 6 * scale + floatOff);
        ctx.stroke();

        // Hooded head
        ctx.fillStyle = `rgba(100, 80, 130, 0.6)`;
        ctx.beginPath();
        ctx.arc(cx, baseY - 12 * scale + floatOff, 6 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Purple glowing eyes
        const eyePulse = animated ? 0.6 + Math.sin(t * 3.5) * 0.4 : 0.7;
        ctx.fillStyle = `rgba(179, 157, 219, ${eyePulse})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 13 * scale + floatOff, 1.8 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 13 * scale + floatOff, 1.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(220, 200, 255, ${eyePulse * 0.4})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 13 * scale + floatOff, 4 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 13 * scale + floatOff, 4 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (animated) {
          for (let i = 0; i < 3; i++) {
            const wispPhase = (t * 2 + i * 1.2) % 1;
            const wx = cx + Math.sin(t * 3 + i * 2) * 8 * scale;
            const wy = baseY - 4 * scale + floatOff - wispPhase * 16 * scale;
            ctx.fillStyle = `rgba(179, 157, 219, ${(1 - wispPhase) * 0.3})`;
            ctx.beginPath();
            ctx.arc(wx, wy, (1 - wispPhase) * 2 * scale, 0, Math.PI * 2);
            ctx.fill();
          }
        }
      ...
      case "bone_mage": {
        // Skeleton mage with purple energy, skull-topped staff, spell circle
        const boneColor = "#d4c5a0";
        const magicColor = "#ce93d8";
        const baseY = cy + 8 * scale - bounce;

        // Spell circle on ground
        if (animated) {
          ctx.strokeStyle = `rgba(206, 147, 216, ${0.3 + Math.sin(t * 2) * 0.15})`;
          ctx.lineWidth = 1 * scale;
          ctx.beginPath();
          ctx.ellipse(cx, baseY + 10 * scale, 12 * scale, 4 * scale, 0, 0, Math.PI * 2);
          ctx.stroke();
          ctx.beginPath();
          const circRot = t * 0.5;
          for (let i = 0; i < 6; i++) {
            const a = circRot + (i * Math.PI * 2) / 6;
            const rx = cx + Math.cos(a) * 10 * scale;
            const ry = baseY + 10 * scale + Math.sin(a) * 3 * scale;
            ctx.moveTo(rx, ry);
            const na = circRot + ((i + 2) * Math.PI * 2) / 6;
            ctx.lineTo(cx + Math.cos(na) * 10 * scale, baseY + 10 * scale + Math.sin(na) * 3 * scale);
          }
          ctx.stroke();
        }
        ...
        ctx.lineTo(cx - 5 * scale, baseY - 4 * scale);
        ctx.lineTo(cx + 5 * scale, baseY - 4 * scale);
        ctx.lineTo(cx + 7 * scale, baseY + 10 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 6 * scale, 5 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Skull head
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale, 5.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        // Eyes with purple glow
        const mageGlow = animated ? 0.6 + Math.sin(t * 3) * 0.4 : 0.8;
        ctx.fillStyle = `rgba(206, 147, 216, ${mageGlow})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Skull-topped staff
        ctx.fillStyle = "#3a2a1a";
        ctx.fillRect(cx - 13 * scale, baseY - 20 * scale, 2 * scale, 26 * scale);
        ctx.fillStyle = boneColor;
        ctx.beginPath();
        ctx.arc(cx - 12 * scale, baseY - 22 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#1a1a1a";
        ctx.beginPath();
        ctx.arc(cx - 13 * scale, baseY - 22.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(cx - 11 * scale, baseY - 22.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Purple energy orb in hand
        ctx.fillStyle = `rgba(206, 147, 216, ${mageGlow * 0.5})`;
        ctx.beginPath();
        ctx.arc(cx + 8 * scale, baseY - 6 * scale, 4 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(255, 220, 255, ${mageGlow * 0.4})`;
        ctx.beginPath();
        ctx.arc(cx + 8 * scale, baseY - 6 * scale, 2 * scale, 0, Math.PI * 2);
        ctx.fill();

        break;
      }
      case "dark_priest": {
        // Hooded dark robes, glowing red symbol, censer
        const robeColor = "#1a0a0a";
        const symbolColor = "#5c1a1a";
        const baseY = cy + 6 * scale - bounce;

        // Robes (full length, no visible legs)
        ctx.fillStyle = robeColor;
        ctx.beginPath();
        ctx.moveTo(cx - 9 * scale, baseY + 12 * scale);
        ctx.quadraticCurveTo(cx - 8 * scale, baseY, cx - 6 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 6 * scale, baseY - 8 * scale);
        ctx.quadraticCurveTo(cx + 8 * scale, baseY, cx + 9 * scale, baseY + 12 * scale);
        ctx.closePath();
        ctx.fill();

        // Glowing red symbol on chest
        const symbolGlow = animated ? 0.6 + Math.sin(t * 2) * 0.3 : 0.7;
        ctx.strokeStyle = `rgba(200, 50, 50, ${symbolGlow})`;
        ctx.lineWidth = 1.2 * scale;
        ctx.beginPath();
        ctx.moveTo(cx, baseY - 6 * scale);
        ctx.lineTo(cx - 3 * scale, baseY);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx, baseY + 1 * scale);
        ctx.lineTo(cx - 3 * scale, baseY - 5 * scale);
        ctx.lineTo(cx + 3 * scale, baseY - 5 * scale);
        ctx.closePath();
        ctx.stroke();
        ctx.fillStyle = `rgba(200, 50, 50, ${symbolGlow * 0.15})`;
        ctx.beginPath();
        ctx.arc(cx, baseY - 3 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(cx, baseY - 13 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Red eyes from hood
        ctx.fillStyle = `rgba(200, 50, 50, ${symbolGlow})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 14 * scale, 1.2 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 14 * scale, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Censer (swinging from right hand)
        const censerSwing = animated ? Math.sin(t * 2) * 0.3 : 0;
        ctx.save();
        ctx.translate(cx + 10 * scale, baseY - 4 * scale);
        ctx.rotate(censerSwing);
        ...
        if (animated) {
          for (let i = 0; i < 3; i++) {
            const smokePhase = (t * 1.5 + i * 0.5) % 1;
            ctx.fillStyle = `rgba(100, 50, 50, ${(1 - smokePhase) * 0.3})`;
            ctx.beginPath();
            ctx.arc(
              Math.sin(t * 2 + i) * 2 * scale,
              10 * scale - smokePhase * 10 * scale,
              (1 + smokePhase) * 1.5 * scale,
              0,
              Math.PI * 2,
            );
            ctx.fill();
      ...
      case "revenant": {
        // Vengeful warrior, spectral translucent armor, red eyes, ghostly weapon
        const ghostColor = "rgba(180, 180, 200, 0.4)";
        const fireColor = "#c62828";
        const baseY = cy + 6 * scale - bounce;
        const floatOff = animated ? Math.sin(t * 2.5) * 2 * scale : 0;

        // Spectral fire aura
        if (animated) {
          ctx.fillStyle = `rgba(198, 40, 40, ${0.1 + Math.sin(t * 3) * 0.05})`;
          ctx.beginPath();
          ctx.arc(cx, baseY - 4 * scale + floatOff, 16 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Translucent legs
        ctx.fillStyle = ghostColor;
        ctx.fillRect(cx - 5 * scale, baseY + 2 * scale + floatOff, 3.5 * scale, 8 * scale);
        ctx.fillRect(cx + 1.5 * scale, baseY + 2 * scale + floatOff, 3.5 * scale, 8 * scale);

        // Spectral armor torso
        ctx.fillStyle = "rgba(160, 160, 180, 0.45)";
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY + 2 * scale + floatOff);
        ctx.lineTo(cx - 7 * scale, baseY - 12 * scale + floatOff);
        ctx.lineTo(cx + 7 * scale, baseY - 12 * scale + floatOff);
        ctx.lineTo(cx + 8 * scale, baseY + 2 * scale + floatOff);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.ellipse(cx - 9 * scale, baseY - 11 * scale + floatOff, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 9 * scale, baseY - 11 * scale + floatOff, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(cx, baseY - 16 * scale + floatOff, 6 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Burning red eyes
        const redGlow = animated ? 0.7 + Math.sin(t * 4) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(198, 40, 40, ${redGlow})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 17 * scale + floatOff, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 17 * scale + floatOff, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        // Eye trail
        if (animated) {
          ctx.fillStyle = `rgba(198, 40, 40, ${redGlow * 0.2})`;
          ctx.beginPath();
          ctx.moveTo(cx - 3 * scale, baseY - 17 * scale + floatOff);
          ctx.lineTo(cx - 6 * scale, baseY - 19 * scale + floatOff);
          ctx.lineTo(cx - 2 * scale, baseY - 18 * scale + floatOff);
          ctx.closePath();
          ctx.fill();
          ctx.beginPath();
          ctx.moveTo(cx + 3 * scale, baseY - 17 * scale + floatOff);
        ...
        ctx.fillRect(cx + 11 * scale, baseY - 22 * scale + floatOff, 2 * scale, 26 * scale);
        ctx.fillStyle = `rgba(255, 100, 100, ${redGlow * 0.3})`;
        ctx.fillRect(cx + 8 * scale, baseY - 2 * scale + floatOff, 8 * scale, 2 * scale);

        break;
      }
      case "abomination": {
        // BOSS monster: Multiple arms (4), mismatched body, stitches, chains, hulking
        const skinColor = "#4e342e";
        const skinDark = "#3a2520";
        const stitchColor = "#222";
        const baseY = cy + 4 * scale - bounce;

        // Massive legs
        ctx.fillStyle = skinDark;
        ctx.fillRect(cx - 7 * scale, baseY + 4 * scale, 5 * scale, 10 * scale);
        ctx.fillRect(cx + 2 * scale, baseY + 4 * scale, 5 * scale, 10 * scale);

        // Hulking body
        ctx.fillStyle = skinColor;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 4 * scale, 12 * scale, 14 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineTo(cx - 2 * scale, baseY + 8 * scale);
        ctx.stroke();
        for (let i = 0; i < 7; i++) {
          const sy = baseY - 14 * scale + i * 3 * scale;
          const sx = cx - 1 * scale + (i * -0.3) * scale;
          ctx.beginPath();
          ctx.moveTo(sx - 2 * scale, sy - 1 * scale);
        ...
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.moveTo(cx - 10 * scale, baseY - 8 * scale);
        ctx.lineTo(cx + 10 * scale, baseY);
        ctx.stroke();

        // 4 Arms (2 upper massive, 2 lower smaller)
        ctx.fillStyle = skinColor;
        // Upper left
        ctx.save();
        ctx.translate(cx - 12 * scale, baseY - 10 * scale);
        ctx.rotate(0.3 + (animated ? Math.sin(t * 2) * 0.1 : 0));
        ctx.fillRect(-3 * scale, 0, 5 * scale, 14 * scale);
        ctx.fillStyle = skinDark;
        ctx.beginPath();
        ...
        ctx.save();
        ctx.translate(cx + 12 * scale, baseY - 10 * scale);
        ctx.rotate(-0.3 + (animated ? Math.sin(t * 2 + 1) * 0.1 : 0));
        ctx.fillRect(-2 * scale, 0, 5 * scale, 14 * scale);
        ctx.fillStyle = skinDark;
        ctx.beginPath();
        ctx.arc(0, 15 * scale, 3 * scale, 0, Math.PI * 2);
        ...
        ctx.fillStyle = skinDark;
        ctx.save();
        ctx.translate(cx - 10 * scale, baseY - 2 * scale);
        ctx.rotate(0.5 + (animated ? Math.sin(t * 2.5) * 0.1 : 0));
        ctx.fillRect(-1.5 * scale, 0, 3 * scale, 10 * scale);
        ctx.restore();
        // Lower right (smaller)
        ctx.save();
        ctx.translate(cx + 10 * scale, baseY - 2 * scale);
        ctx.rotate(-0.5 + (animated ? Math.sin(t * 2.5 + 1) * 0.1 : 0));
        ctx.fillRect(-1.5 * scale, 0, 3 * scale, 10 * scale);
        ctx.restore();
        ...
        ctx.fillStyle = skinDark;
        ctx.beginPath();
        ctx.ellipse(cx + 2 * scale, baseY - 18 * scale, 5 * scale, 4.5 * scale, 0.15, 0, Math.PI * 2);
        ctx.fill();

        // Asymmetric eyes
        ctx.fillStyle = "#ff4444";
        ctx.beginPath();
        ctx.arc(cx, baseY - 19 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#ffaa00";
        ctx.beginPath();
        ctx.arc(cx + 4 * scale, baseY - 18.5 * scale, 1 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Drooling mouth
        ctx.fillStyle = "#1a0a0a";
        ctx.beginPath();
        ctx.ellipse(cx + 2 * scale, baseY - 15.5 * scale, 3 * scale, 1.5 * scale, 0, 0, Math.PI);
        ctx.fill();

        break;
      }
      case "hellhound": {
        // Demonic quadruped, flaming mane, red eyes, fangs, fiery tail
        const bodyColor = "#3a1a0a";
        ...
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 4 * scale, 10 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Legs (4)
        ctx.fillStyle = "#2a0a00";
        ctx.fillRect(cx - 8 * scale, baseY, 2.5 * scale, 8 * scale);
        ctx.fillRect(cx - 4 * scale, baseY + 1 * scale, 2.5 * scale, 7 * scale);
        ctx.fillRect(cx + 3 * scale, baseY + 1 * scale, 2.5 * scale, 7 * scale);
        ctx.fillRect(cx + 7 * scale, baseY, 2.5 * scale, 8 * scale);

        // Head
        ctx.fillStyle = bodyColor;
        ctx.beginPath();
        ctx.ellipse(cx - 10 * scale, baseY - 6 * scale, 5 * scale, 4.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();

        // Snout
        ctx.fillStyle = "#2a0a00";
        ctx.beginPath();
        ctx.ellipse(cx - 14 * scale, baseY - 5 * scale, 3 * scale, 2 * scale, -0.1, 0, Math.PI * 2);
        ctx.fill();

        // Fangs
        ctx.fillStyle = "#ddd";
        ctx.beginPath();
        ctx.moveTo(cx - 15 * scale, baseY - 4 * scale);
        ctx.lineTo(cx - 15.5 * scale, baseY - 2 * scale);
        ctx.lineTo(cx - 14.5 * scale, baseY - 2.5 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(cx - 13 * scale, baseY - 4 * scale);
        ctx.lineTo(cx - 13.5 * scale, baseY - 2 * scale);
        ctx.lineTo(cx - 12.5 * scale, baseY - 2.5 * scale);
        ctx.closePath();
        ctx.fill();

        // Red glowing eyes
        const eyeFire = animated ? 0.7 + Math.sin(t * 5) * 0.3 : 0.8;
        ctx.fillStyle = `rgba(255, 50, 0, ${eyeFire})`;
        ctx.beginPath();
        ctx.arc(cx - 11 * scale, baseY - 7.5 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.arc(cx - 9 * scale, baseY - 7.5 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Flaming mane
        if (animated) {
          for (let i = 0; i < 5; i++) {
            const fPhase = (t * 4 + i * 0.5) % 1;
            const fx = cx - 6 * scale + i * 3 * scale;
            const fy = baseY - 8 * scale - fPhase * 6 * scale;
            ctx.fillStyle = `rgba(255, 111, 0, ${(1 - fPhase) * 0.6})`;
            ctx.beginPath();
            ctx.ellipse(fx, fy, 2 * scale, (2 + fPhase * 2) * scale, 0, 0, Math.PI * 2);
            ctx.fill();
          }
        } else {
          ...
          for (let i = 0; i < 4; i++) {
            ctx.beginPath();
            ctx.ellipse(cx - 5 * scale + i * 3 * scale, baseY - 10 * scale, 2 * scale, 3 * scale, 0, 0, Math.PI * 2);
            ctx.fill();
          }
        }
        ...
        ctx.lineWidth = 2 * scale;
        const tailSwing = animated ? Math.sin(t * 3) * 0.3 : 0;
        ctx.beginPath();
        ctx.moveTo(cx + 10 * scale, baseY - 4 * scale);
        ctx.quadraticCurveTo(cx + 14 * scale, baseY - 8 * scale + tailSwing * 4 * scale, cx + 16 * scale, baseY - 12 * scale);
        ctx.stroke();
        if (animated) {
          ctx.fillStyle = `rgba(255, 200, 50, ${0.5 + Math.sin(t * 6) * 0.3})`;
          ctx.beginPath();
          ctx.arc(cx + 16 * scale, baseY - 12 * scale, 2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      case "doom_herald": {
        // BOSS flying dark angel: Large dark wings, void halo, robes, staff, trails
        const robeColor = "#1a0a2e";
        const wingColor = "#0d0d1a";
        const voidColor = "#311b92";
        const baseY = cy + 4 * scale - bounce;
        const floatOff = animated ? Math.sin(t * 2) * 3 * scale : 0;

        // Void aura
        if (animated) {
          ctx.fillStyle = `rgba(49, 27, 146, ${0.1 + Math.sin(t * 2) * 0.05})`;
          ctx.beginPath();
          ctx.arc(cx, baseY - 6 * scale + floatOff, 24 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Large dark wings
        const wingFlap = animated ? Math.sin(t * 3) * 0.15 : 0;
        ctx.fillStyle = wingColor;
        // Left wing
        ctx.save();
        ctx.translate(cx - 6 * scale, baseY - 10 * scale + floatOff);
        ctx.rotate(-0.3 + wingFlap);
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.quadraticCurveTo(-14 * scale, -8 * scale, -20 * scale, -2 * scale);
        ctx.quadraticCurveTo(-16 * scale, 4 * scale, -10 * scale, 6 * scale);
        ctx.quadraticCurveTo(-6 * scale, 4 * scale, 0, 4 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.restore();
        // Right wing
        ...
        ctx.fillStyle = robeColor;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, baseY + 14 * scale + floatOff);
        ctx.quadraticCurveTo(cx - 7 * scale, baseY + 2 * scale + floatOff, cx - 5 * scale, baseY - 8 * scale + floatOff);
        ctx.lineTo(cx + 5 * scale, baseY - 8 * scale + floatOff);
        ctx.quadraticCurveTo(cx + 7 * scale, baseY + 2 * scale + floatOff, cx + 8 * scale, baseY + 14 * scale + floatOff);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(cx, baseY - 14 * scale + floatOff, 6 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Void eyes
        const voidPulse = animated ? 0.6 + Math.sin(t * 3) * 0.4 : 0.8;
        ctx.fillStyle = `rgba(49, 27, 146, ${voidPulse})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale + floatOff, 1.8 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale + floatOff, 1.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(120, 80, 255, ${voidPulse * 0.3})`;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, baseY - 15 * scale + floatOff, 4 * scale, 0, Math.PI * 2);
        ctx.arc(cx + 2 * scale, baseY - 15 * scale + floatOff, 4 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Void halo (dark, inverted)
        ctx.strokeStyle = `rgba(49, 27, 146, ${voidPulse * 0.7})`;
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 22 * scale + floatOff, 7 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.strokeStyle = `rgba(80, 50, 180, ${voidPulse * 0.3})`;
        ctx.lineWidth = 3 * scale;
        ctx.beginPath();
        ctx.ellipse(cx, baseY - 22 * scale + floatOff, 8 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.stroke();

        // Staff
        ctx.fillStyle = "#1a0a2e";
        ctx.fillRect(cx + 10 * scale, baseY - 20 * scale + floatOff, 2 * scale, 28 * scale);
        ctx.fillStyle = `rgba(49, 27, 146, ${voidPulse})`;
        ctx.beginPath();
        ctx.arc(cx + 11 * scale, baseY - 22 * scale + floatOff, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(180, 140, 255, ${voidPulse * 0.5})`;
        ctx.beginPath();
        ctx.arc(cx + 11 * scale, baseY - 22 * scale + floatOff, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Shadow trails
        if (animated) {
          for (let i = 0; i < 4; i++) {
            const trailPhase = (t * 1.5 + i * 0.4) % 1;
            ctx.fillStyle = `rgba(49, 27, 146, ${(1 - trailPhase) * 0.15})`;
            ctx.beginPath();
            ctx.ellipse(
              cx + Math.sin(t * 2 + i) * 4 * scale,
              baseY + 10 * scale + floatOff + trailPhase * 10 * scale,
              (3 - trailPhase * 2) * scale,
              (1.5 - trailPhase) * scale,
              0,
              0,
              Math.PI * 2,
            );
      ...
      // =====================================================
      // REGIONAL BOSS ENEMIES
      // =====================================================

      case "titan_of_nassau": {
        const bossColor = "#e65100";
        const stoneColor = "#6d6d6d";
        const runeGlow = 0.5 + Math.sin(t * 3) * 0.5;
        const auraPulse = 0.6 + Math.sin(t * 1.5) * 0.4;
        const crackFlicker = 0.3 + Math.sin(t * 5) * 0.2;

        // Ground cracks
        ctx.strokeStyle = `rgba(230, 81, 0, ${crackFlicker})`;
        ctx.lineWidth = 1.2 * scale;
        for (let i = 0; i < 5; i++) {
          const angle = (i / 5) * Math.PI * 2 + t * 0.3;
          const len = (8 + Math.sin(t * 2 + i) * 3) * scale;
          ctx.beginPath();
          ctx.moveTo(cx, cy + 8 * scale);
          ctx.lineTo(cx + Math.cos(angle) * len, cy + 8 * scale + Math.sin(angle) * len * 0.4);
          ctx.stroke();
        }

        // Princeton orange aura
        const auraGrad = ctx.createRadialGradient(cx, cy - bounce, 2 * scale, cx, cy - bounce, 18 * scale);
        auraGrad.addColorStop(0, `rgba(230, 81, 0, ${auraPulse * 0.3})`);
        auraGrad.addColorStop(0.6, `rgba(255, 152, 0, ${auraPulse * 0.15})`);
        auraGrad.addColorStop(1, "rgba(230, 81, 0, 0)");
        ctx.fillStyle = auraGrad;
        ctx.beginPath();
        ctx.arc(cx, cy - bounce, 18 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Quadruped body (stone)
        ctx.fillStyle = stoneColor;
        ctx.beginPath();
        ctx.ellipse(cx, cy + 2 * scale - bounce, 12 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#555";
        ctx.lineWidth = 0.5 * scale;
        ctx.stroke();
        ...
        // Stone legs
        const legPositions = [-8, -3, 3, 8];
        for (let i = 0; i < 4; i++) {
          const legSway = Math.sin(t * 3 + i * Math.PI * 0.5) * 1.5 * scale;
          ctx.fillStyle = stoneColor;
          ctx.fillRect(cx + legPositions[i] * scale - 1.5 * scale, cy + 6 * scale - bounce + legSway, 3 * scale, 7 * scale);
          ctx.fillStyle = "#555";
          ctx.fillRect(cx + legPositions[i] * scale - 2 * scale, cy + 12 * scale - bounce + legSway, 4 * scale, 2 * scale);
        }

        // Stone mane with energy
        for (let i = 0; i < 8; i++) {
          const maneAngle = -Math.PI * 0.8 + (i / 7) * Math.PI * 0.6;
          const maneLen = (6 + Math.sin(t * 2 + i) * 2) * scale;
          const energyAlpha = 0.4 + Math.sin(t * 4 + i * 0.8) * 0.3;
          ctx.strokeStyle = `rgba(255, 152, 0, ${energyAlpha})`;
          ctx.lineWidth = 2 * scale;
          ctx.beginPath();
          ctx.moveTo(cx - 2 * scale, cy - 4 * scale - bounce);
          ctx.lineTo(
            cx - 2 * scale + Math.cos(maneAngle) * maneLen,
            cy - 4 * scale - bounce + Math.sin(maneAngle) * maneLen,
          );
          ctx.stroke();
        }
        ...
        ctx.fillStyle = stoneColor;
        ctx.beginPath();
        ctx.arc(cx - 1 * scale, cy - 5 * scale - bounce, 6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#555";
        ctx.lineWidth = 0.5 * scale;
        ctx.stroke();

        // Golden rune veins (animated glow)
        ctx.strokeStyle = `rgba(255, 215, 0, ${runeGlow})`;
        ctx.lineWidth = 1 * scale;
        ctx.shadowColor = "#ffd700";
        ctx.shadowBlur = 4 * scale * runeGlow;
        for (let i = 0; i < 4; i++) {
          const rx = cx + (Math.cos(i * 1.5 + 0.5) * 8 - 4) * scale;
          const ry = cy + (Math.sin(i * 1.2) * 4 - 1) * scale - bounce;
          ctx.beginPath();
          ctx.moveTo(rx, ry);
          ctx.quadraticCurveTo(rx + 3 * scale, ry - 2 * scale, rx + 5 * scale, ry + 1 * scale);
          ctx.stroke();
        }
        ctx.shadowBlur = 0;

        // Fire eyes
        const eyeGlow = 0.8 + Math.sin(t * 6) * 0.2;
        ctx.fillStyle = `rgba(255, 100, 0, ${eyeGlow})`;
        ctx.shadowColor = bossColor;
        ctx.shadowBlur = 6 * scale;
        ctx.beginPath();
        ctx.arc(cx - 4 * scale, cy - 6.5 * scale - bounce, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(cx + 1.5 * scale, cy - 6.5 * scale - bounce, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
      ...
      case "swamp_leviathan": {
        const bossColor = "#1b5e20";
        const scaleColor = "#2e7d32";
        const bileGreen = "#76ff03";
        const toxicPulse = 0.5 + Math.sin(t * 2) * 0.5;
        const mistAlpha = 0.15 + Math.sin(t * 1.2) * 0.1;

        // Toxic mist aura
        for (let ring = 3; ring >= 0; ring--) {
          ctx.fillStyle = `rgba(27, 94, 32, ${mistAlpha * (1 - ring * 0.2)})`;
          ctx.beginPath();
          ctx.arc(cx, cy - bounce, (14 + ring * 3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Dripping bile particles
        for (let i = 0; i < 6; i++) {
          const px = cx + (Math.sin(t * 1.5 + i * 1.2) * 10) * scale;
          const py = cy + ((t * 8 + i * 12) % 24 - 12) * scale - bounce;
          const pAlpha = 0.3 + Math.sin(t * 3 + i) * 0.2;
          ctx.fillStyle = `rgba(118, 255, 3, ${pAlpha})`;
          ctx.beginPath();
          ctx.arc(px, py, (1 + Math.sin(t + i) * 0.5) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        for (let i = 0; i <= 20; i++) {
          const frac = i / 20;
          const bx = cx + Math.sin(frac * Math.PI * 3 + t) * 8 * scale;
          const by = cy + (frac * 16 - 6) * scale - bounce;
        ...
        for (let i = 0; i < 12; i++) {
          const frac = i / 12;
          const sx = cx + Math.sin(frac * Math.PI * 3 + t) * 8 * scale;
          const sy = cy + (frac * 16 - 6) * scale - bounce;
          ctx.beginPath();
          ctx.arc(sx, sy, 3 * scale, 0, Math.PI, true);
          ctx.stroke();
        }

        // Three necks + heads
        for (let h = 0; h < 3; h++) {
          const neckAngle = (h - 1) * 0.5 + Math.sin(t * 2 + h * 2) * 0.2;
          const neckLen = (10 + Math.sin(t * 1.5 + h) * 2) * scale;
          const headX = cx + Math.sin(neckAngle) * neckLen;
          const headY = cy - 8 * scale - bounce - Math.cos(neckAngle) * neckLen;

          // Neck
          ctx.strokeStyle = scaleColor;
          ctx.lineWidth = 3.5 * scale;
          ctx.beginPath();
          ctx.moveTo(cx + (h - 1) * 3 * scale, cy - 4 * scale - bounce);
          ctx.quadraticCurveTo(
            cx + (h - 1) * 5 * scale,
            cy - 10 * scale - bounce,
            headX,
          ...
          ctx.beginPath();
          ctx.ellipse(headX, headY, 4 * scale, 3 * scale, neckAngle, 0, Math.PI * 2);
          ctx.fill();
          ...
          ctx.beginPath();
          ctx.ellipse(
            headX + Math.sin(neckAngle) * 2.5 * scale,
            headY - Math.cos(neckAngle) * 2.5 * scale,
            3 * scale,
            1.5 * scale,
            neckAngle,
            0,
          ...
          // Glowing eyes
          ctx.fillStyle = `rgba(118, 255, 3, ${toxicPulse})`;
          ctx.shadowColor = bileGreen;
          ctx.shadowBlur = 5 * scale;
          ctx.beginPath();
          ctx.arc(headX - 1.5 * scale, headY - 1 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.beginPath();
          ctx.arc(headX + 1.5 * scale, headY - 1 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
      ...
      case "sphinx_guardian": {
        ...
        const eyePulse = 0.6 + Math.sin(t * 4) * 0.4;
        const symbolOrbit = t * 1.5;
        const wingFold = 0.1 + Math.sin(t * 1) * 0.05;

        // Stone base / pedestal glow
        ctx.fillStyle = `rgba(249, 168, 37, 0.12)`;
        ctx.beginPath();
        ctx.ellipse(cx, cy + 10 * scale - bounce, 14 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Sitting lion body
        ctx.fillStyle = goldColor;
        ctx.beginPath();
        ctx.ellipse(cx + 1 * scale, cy + 3 * scale - bounce, 10 * scale, 6 * scale, -0.1, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Front legs (tucked)
        ctx.fillStyle = goldColor;
        ctx.fillRect(cx - 7 * scale, cy + 5 * scale - bounce, 4 * scale, 6 * scale);
        ctx.fillRect(cx + 3 * scale, cy + 5 * scale - bounce, 4 * scale, 6 * scale);
        ctx.strokeStyle = darkGold;
        ctx.lineWidth = 0.5 * scale;
        ctx.strokeRect(cx - 7 * scale, cy + 5 * scale - bounce, 4 * scale, 6 * scale);
        ctx.strokeRect(cx + 3 * scale, cy + 5 * scale - bounce, 4 * scale, 6 * scale);

        // Paws
        ctx.fillStyle = darkGold;
        ctx.fillRect(cx - 8 * scale, cy + 10 * scale - bounce, 6 * scale, 2 * scale);
        ctx.fillRect(cx + 2 * scale, cy + 10 * scale - bounce, 6 * scale, 2 * scale);

        // Stone wings (folded at sides)
        for (let side = -1; side <= 1; side += 2) {
          ctx.fillStyle = `rgba(193, 121, 0, 0.5)`;
          ctx.beginPath();
          ctx.moveTo(cx + side * 9 * scale, cy - 2 * scale - bounce);
          ctx.quadraticCurveTo(
            cx + side * 16 * scale,
            cy - 6 * scale - bounce + Math.sin(t + side) * wingFold * 20 * scale,
            cx + side * 14 * scale,
            cy + 4 * scale - bounce,
          );
          ctx.lineTo(cx + side * 9 * scale, cy + 5 * scale - bounce);
          ctx.closePath();
          ctx.fill();
          ctx.strokeStyle = darkGold;
          ctx.lineWidth = 0.5 * scale;
        ...
        ctx.fillStyle = goldColor;
        ctx.beginPath();
        ctx.arc(cx, cy - 7 * scale - bounce, 5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = darkGold;
        ctx.lineWidth = 0.5 * scale;
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.moveTo(cx - 6 * scale, cy - 7 * scale - bounce);
        ctx.lineTo(cx, cy - 13 * scale - bounce);
        ctx.lineTo(cx + 6 * scale, cy - 7 * scale - bounce);
        ctx.closePath();
        ctx.fill();
        ...
        for (let side = -1; side <= 1; side += 2) {
          ctx.beginPath();
          ctx.moveTo(cx + side * 5.5 * scale, cy - 6 * scale - bounce);
          ctx.lineTo(cx + side * 7 * scale, cy + 0 * scale - bounce);
          ctx.lineTo(cx + side * 4 * scale, cy + 0 * scale - bounce);
          ctx.closePath();
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#333";
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, cy - 8 * scale - bounce, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(cx + 2 * scale, cy - 8 * scale - bounce, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.shadowBlur = 8 * scale * eyePulse;
        ctx.beginPath();
        ctx.arc(cx, cy - 10 * scale - bounce, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;

        // Orbiting hieroglyphic symbols
        for (let i = 0; i < 5; i++) {
          const orbitAngle = symbolOrbit + (i / 5) * Math.PI * 2;
          const orbitR = 16 * scale;
          const sx = cx + Math.cos(orbitAngle) * orbitR;
          const sy = cy - 2 * scale - bounce + Math.sin(orbitAngle) * orbitR * 0.4;
          const symAlpha = 0.4 + Math.sin(t * 2 + i) * 0.3;
          ctx.fillStyle = `rgba(255, 215, 0, ${symAlpha})`;
          ctx.font = `${3 * scale}px serif`;
          const symbols = ["𓂀", "𓁹", "𓆣", "𓋴", "𓊝"];
      ...
      case "frost_colossus": {
        const iceBlue = "#4dd0e1";
        const deepIce = "#00838f";
        const frostAlpha = 0.4 + Math.sin(t * 1.5) * 0.2;
        const crystalRotate = t * 0.8;
        const crownPulse = 0.6 + Math.sin(t * 2.5) * 0.4;

        // Frost particle aura
        for (let i = 0; i < 10; i++) {
          const angle = (i / 10) * Math.PI * 2 + t * 0.5;
          const dist = (12 + Math.sin(t * 2 + i * 1.3) * 4) * scale;
          const px = cx + Math.cos(angle) * dist;
          const py = cy - 2 * scale - bounce + Math.sin(angle) * dist * 0.5;
          const pAlpha = 0.2 + Math.sin(t * 3 + i) * 0.15;
          ctx.fillStyle = `rgba(255, 255, 255, ${pAlpha})`;
          ctx.beginPath();
          ctx.arc(px, py, (1 + Math.sin(t * 2 + i) * 0.5) * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Translucent ice body
        const bodyGrad = ctx.createLinearGradient(cx, cy - 14 * scale - bounce, cx, cy + 10 * scale - bounce);
        bodyGrad.addColorStop(0, `rgba(77, 208, 225, 0.7)`);
        bodyGrad.addColorStop(0.5, `rgba(0, 131, 143, 0.8)`);
        bodyGrad.addColorStop(1, `rgba(77, 208, 225, 0.6)`);

        // Torso
        ctx.fillStyle = bodyGrad;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * scale, cy - 4 * scale - bounce);
        ...
        // Legs
        ctx.fillStyle = bodyGrad;
        ctx.fillRect(cx - 7 * scale, cy + 6 * scale - bounce, 5 * scale, 8 * scale);
        ctx.fillRect(cx + 2 * scale, cy + 6 * scale - bounce, 5 * scale, 8 * scale);

        // Floating crystal armor plates
        for (let i = 0; i < 4; i++) {
          const plateAngle = crystalRotate + (i / 4) * Math.PI * 2;
          const plateDist = 6 * scale;
          const px = cx + Math.cos(plateAngle) * plateDist;
          const py = cy - bounce + Math.sin(plateAngle) * plateDist * 0.3;
          ctx.fillStyle = `rgba(77, 208, 225, ${0.4 + Math.sin(t + i) * 0.2})`;
          ctx.save();
          ctx.translate(px, py);
        ...
        for (let side = -1; side <= 1; side += 2) {
          ...
          ctx.save();
          ctx.translate(cx + side * 9 * scale, cy - 2 * scale - bounce);
          ctx.rotate(side * 0.2 + armSwing * side);
          ctx.fillRect(-2 * scale, 0, 4 * scale, 10 * scale);
          ctx.restore();
        }
        ...
        ctx.beginPath();
        ctx.arc(cx, cy - 8 * scale - bounce, 5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Icicle beard
        for (let i = 0; i < 5; i++) {
          const bx = cx + (i - 2) * 2 * scale;
          const bLen = (3 + Math.sin(t + i) * 1) * scale;
          ctx.fillStyle = `rgba(200, 240, 255, 0.7)`;
          ctx.beginPath();
          ctx.moveTo(bx - 0.8 * scale, cy - 4 * scale - bounce);
          ctx.lineTo(bx + 0.8 * scale, cy - 4 * scale - bounce);
          ctx.lineTo(bx, cy - 4 * scale - bounce + bLen);
          ctx.closePath();
          ctx.fill();
        }

        // Ice crown (jagged)
        ctx.fillStyle = `rgba(200, 240, 255, ${crownPulse})`;
        ctx.shadowColor = iceBlue;
        ctx.shadowBlur = 6 * scale * crownPulse;
        for (let i = 0; i < 5; i++) {
          const crownX = cx + (i - 2) * 2.5 * scale;
          const crownH = (4 + Math.sin(i * 1.5) * 2) * scale;
          ctx.beginPath();
          ctx.moveTo(crownX - 1.2 * scale, cy - 12 * scale - bounce);
          ctx.lineTo(crownX, cy - 12 * scale - bounce - crownH);
          ctx.lineTo(crownX + 1.2 * scale, cy - 12 * scale - bounce);
          ctx.closePath();
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        ctx.shadowBlur = 6 * scale;
        ctx.beginPath();
        ctx.arc(cx - 2 * scale, cy - 9 * scale - bounce, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(cx + 2 * scale, cy - 9 * scale - bounce, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();
      ...
      case "inferno_wyrm": {
        const lavaRed = "#d50000";
        const moltenOrange = "#ff6d00";
        const emberYellow = "#ffd600";
        const firePulse = 0.5 + Math.sin(t * 3) * 0.5;
        const magmaGlow = 0.6 + Math.sin(t * 2) * 0.4;

        // Ember particles rising
        for (let i = 0; i < 12; i++) {
          const ex = cx + (Math.sin(t * 1.2 + i * 2.1) * 14) * scale;
          const ey = cy - ((t * 10 + i * 8) % 30) * scale - bounce;
          const eAlpha = 0.5 - ((t * 10 + i * 8) % 30) / 60;
          if (eAlpha > 0) {
            ctx.fillStyle = `rgba(255, 214, 0, ${eAlpha})`;
            ctx.beginPath();
            ctx.arc(ex, ey, (0.8 + Math.sin(t * 4 + i) * 0.4) * scale, 0, Math.PI * 2);
            ctx.fill();
          }
        }

        // Heat distortion aura
        const heatGrad = ctx.createRadialGradient(cx, cy - bounce, 3 * scale, cx, cy - bounce, 18 * scale);
        heatGrad.addColorStop(0, `rgba(213, 0, 0, ${magmaGlow * 0.25})`);
        heatGrad.addColorStop(0.5, `rgba(255, 109, 0, ${magmaGlow * 0.12})`);
        heatGrad.addColorStop(1, "rgba(213, 0, 0, 0)");
        ctx.fillStyle = heatGrad;
        ctx.beginPath();
        ctx.arc(cx, cy - bounce, 18 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineCap = "round";
        const bodyPath: Array<[number, number]> = [];
        for (let i = 0; i <= 24; i++) {
          const frac = i / 24;
          const bx = cx + Math.sin(frac * Math.PI * 4 + t * 1.5) * 10 * scale;
          const by = cy + (frac * 20 - 8) * scale - bounce;
          bodyPath.push([bx, by]);
        }
        ...
        ctx.beginPath();
        bodyPath.forEach(([bx, by], i) => (i === 0 ? ctx.moveTo(bx + 1, by + 1) : ctx.lineTo(bx + 1, by + 1)));
        ctx.stroke();

        // Main body
        const bodyGrad2 = ctx.createLinearGradient(cx - 10 * scale, cy - bounce, cx + 10 * scale, cy - bounce);
        bodyGrad2.addColorStop(0, lavaRed);
        bodyGrad2.addColorStop(0.5, "#b71c1c");
        bodyGrad2.addColorStop(1, lavaRed);
        ctx.strokeStyle = bodyGrad2;
        ctx.lineWidth = 7 * scale;
        ctx.beginPath();
        bodyPath.forEach(([bx, by], i) => (i === 0 ? ctx.moveTo(bx, by) : ctx.lineTo(bx, by)));
        ctx.stroke();

        // Glowing orange scale gaps
        ctx.strokeStyle = `rgba(255, 109, 0, ${firePulse * 0.7})`;
        ctx.lineWidth = 2 * scale;
        ...
        // Fire mane along spine
        for (let i = 0; i < 10; i++) {
          const idx = Math.floor((i / 10) * bodyPath.length);
          if (idx >= bodyPath.length) continue;
          const [fx, fy] = bodyPath[idx];
          const flameH = (4 + Math.sin(t * 5 + i * 0.9) * 2) * scale;
          const flameAlpha = 0.5 + Math.sin(t * 4 + i) * 0.3;
          const flameGrad = ctx.createLinearGradient(fx, fy, fx, fy - flameH);
          flameGrad.addColorStop(0, `rgba(255, 109, 0, ${flameAlpha})`);
          flameGrad.addColorStop(1, `rgba(255, 214, 0, 0)`);
          ctx.fillStyle = flameGrad;
          ctx.beginPath();
          ctx.moveTo(fx - 2 * scale, fy);
          ctx.lineTo(fx + Math.sin(t * 6 + i) * 1 * scale, fy - flameH);
          ctx.lineTo(fx + 2 * scale, fy);
          ctx.closePath();
          ctx.fill();
        }

        // Wyrm head
        const headX = bodyPath[0][0];
        const headY = bodyPath[0][1];
        ctx.fillStyle = lavaRed;
        ctx.beginPath();
        ctx.ellipse(headX, headY, 5 * scale, 4 * scale, -0.3, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let side = -1; side <= 1; side += 2) {
          ...
          ctx.beginPath();
          ctx.moveTo(headX + side * 3 * scale, headY - 2 * scale);
          ctx.lineTo(headX + side * 5 * scale, headY - 8 * scale);
          ctx.lineTo(headX + side * 2 * scale, headY - 3 * scale);
          ctx.closePath();
          ctx.fill();
          ctx.fillStyle = `rgba(255, 109, 0, ${firePulse})`;
          ctx.beginPath();
          ctx.arc(headX + side * 5 * scale, headY - 8 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Molten eyes
        ctx.fillStyle = `rgba(255, 214, 0, 0.95)`;
        ctx.shadowColor = emberYellow;
        ctx.shadowBlur = 8 * scale;
        ctx.beginPath();
        ctx.arc(headX - 2 * scale, headY - 1.5 * scale, 1.3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(headX + 2 * scale, headY - 1.5 * scale, 1.3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Magma dripping from jaw
        for (let i = 0; i < 3; i++) {
          const dx = headX + (i - 1) * 2 * scale;
          const dripLen = (2 + Math.sin(t * 3 + i * 2) * 2) * scale;
          const dripAlpha = 0.5 + Math.sin(t * 4 + i) * 0.3;
          ctx.fillStyle = `rgba(255, 109, 0, ${dripAlpha})`;
          ctx.beginPath();
          ctx.ellipse(dx, headY + 3 * scale + dripLen * 0.5, 0.8 * scale, dripLen, 0, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      case "orb_weaver": {
        ...
        ctx.lineWidth = 1.5 * scale;
        for (let i = 0; i < 4; i++) {
          const angle = (i * 0.35 + 0.3);
          const legLen = (8 + i * 1.5) * scale;
          const kneeLen = legLen * 0.55;
          const legWave = Math.sin(t * 3 + i * 1.5) * 2 * scale;
          for (const dir of [-1, 1]) {
            const kx = bodyX + dir * kneeLen * Math.cos(angle);
            const ky = bodyY + kneeLen * Math.sin(angle) + legWave;
            const ex = bodyX + dir * legLen * Math.cos(angle - 0.3);
            const ey = bodyY + legLen * Math.sin(angle + 0.2) + legWave * 0.5;
            ctx.beginPath();
        ...
        ctx.lineWidth = 0.5 * scale;
        ctx.beginPath();
        ctx.moveTo(bodyX, bodyY + 6 * scale);
        ctx.quadraticCurveTo(bodyX + Math.sin(t * 2) * 4 * scale, bodyY + 14 * scale, bodyX + Math.sin(t) * 2 * scale, bodyY + 20 * scale);
        ctx.stroke();

        // Abdomen with cross pattern
        ctx.fillStyle = spiderColor;
        ctx.beginPath();
        ctx.ellipse(bodyX, bodyY + 3 * scale, 5 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#8b7355";
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(bodyX, bodyY - 1 * scale);
        ctx.lineTo(bodyX, bodyY + 7 * scale);
        ctx.moveTo(bodyX - 3 * scale, bodyY + 3 * scale);
        ctx.lineTo(bodyX + 3 * scale, bodyY + 3 * scale);
        ctx.stroke();

        // Cephalothorax
        ctx.fillStyle = "#5c4a3a";
        ctx.beginPath();
        ctx.ellipse(bodyX, bodyY - 4 * scale, 3.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dx of [-1.5, -0.5, 0.5, 1.5]) {
          ctx.beginPath();
          ctx.arc(bodyX + dx * scale, bodyY - 5 * scale, 0.6 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        break;
      ...
      case "mantis": {
        ...
        ctx.fillStyle = "#4d7c0f";
        ctx.beginPath();
        ctx.ellipse(bx, by + 6 * scale, 3 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Thorax
        ctx.fillStyle = mantisColor;
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale, 2.5 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Triangular head
        ctx.fillStyle = "#84cc16";
        ctx.beginPath();
        ctx.moveTo(bx, by - 10 * scale);
        ctx.lineTo(bx - 3 * scale, by - 6 * scale);
        ctx.lineTo(bx + 3 * scale, by - 6 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(bx - 2.5 * scale, by - 8 * scale, 1.2 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 2.5 * scale, by - 8 * scale, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Scythe arms (raptorial forelegs)
        ctx.strokeStyle = mantisColor;
        ctx.lineWidth = 2 * scale;
        const armSwing = Math.sin(t * 4) * 0.3;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 2 * scale, by - 3 * scale);
          ctx.lineTo(bx + dir * 7 * scale, by - 8 * scale + armSwing * 3 * scale);
          ctx.lineTo(bx + dir * 4 * scale, by - 13 * scale + armSwing * 5 * scale);
          ctx.stroke();
          // Serrated edge
          ctx.strokeStyle = "#a3e635";
          ctx.lineWidth = 1 * scale;
          ctx.beginPath();
          ctx.moveTo(bx + dir * 7 * scale, by - 8 * scale + armSwing * 3 * scale);
          ctx.lineTo(bx + dir * 4 * scale, by - 13 * scale + armSwing * 5 * scale);
          ctx.stroke();
          ctx.strokeStyle = mantisColor;
          ctx.lineWidth = 2 * scale;
        ...
        for (let i = 0; i < 2; i++) {
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx, by + (2 + i * 4) * scale);
            ctx.lineTo(bx + dir * 6 * scale, by + (5 + i * 3) * scale);
            ctx.lineTo(bx + dir * 4 * scale, by + (10 + i * 2) * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "bombardier_beetle": {
        const beetleColor = "#1e3a5f";
        const bx = cx;
        const by = cy + bounce;

        // Elytra (wing covers) with metallic sheen
        ctx.fillStyle = beetleColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 6 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Metallic highlight
        const grad = ctx.createLinearGradient(bx - 4 * scale, by - 4 * scale, bx + 4 * scale, by + 6 * scale);
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 7 * scale, 3 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 1.5 * scale, by - 9 * scale);
          ctx.quadraticCurveTo(bx + dir * 5 * scale, by - 14 * scale, bx + dir * 3 * scale, by - 12 * scale);
          ctx.stroke();
        }
        ...
        for (let i = 0; i < 3; i++) {
          for (const dir of [-1, 1]) {
            const ly = by + (-2 + i * 3) * scale;
            ctx.beginPath();
            ctx.moveTo(bx + dir * 4 * scale, ly);
            ctx.lineTo(bx + dir * 8 * scale, ly + 3 * scale);
            ctx.stroke();
          }
        }
        ...
        ctx.fillStyle = "#84cc16";
        ctx.beginPath();
        ctx.ellipse(bx, by + 8 * scale, 2 * scale, 1.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Acid spray
        const sprayAlpha = 0.3 + Math.sin(t * 5) * 0.2;
        ctx.fillStyle = `rgba(132, 204, 22, ${sprayAlpha})`;
        for (let i = 0; i < 3; i++) {
          const sx = bx + (Math.sin(t * 4 + i * 2) * 2) * scale;
          const sy = by + (10 + i * 2 + Math.sin(t * 3 + i) * 1) * scale;
          ctx.beginPath();
          ctx.arc(sx, sy, (1.5 - i * 0.3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      }
      ...
      case "mosquito": {
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 5 * scale, by - 3 * scale, 4 * scale, 8 * scale, dir * (0.3 + wingFlap), 0, Math.PI * 2);
          ctx.fill();
          ctx.stroke();
        }

        // Abdomen (blood-filled, dark red)
        ctx.fillStyle = "#6b2142";
        ctx.beginPath();
        ctx.ellipse(bx, by + 4 * scale, 2 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Stripes
        ...
        for (let i = 0; i < 3; i++) {
          ctx.beginPath();
          ctx.moveTo(bx - 1.5 * scale, by + (2 + i * 2) * scale);
          ctx.lineTo(bx + 1.5 * scale, by + (2 + i * 2) * scale);
          ctx.stroke();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale, 2.5 * scale, 2 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#3a1228";
        ctx.beginPath();
        ctx.arc(bx, by - 5 * scale, 2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(bx - 1.5 * scale, by - 5.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.5 * scale, by - 5.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;

        // Long proboscis
        ctx.strokeStyle = "#6b2142";
        ctx.lineWidth = 0.8 * scale;
        ctx.beginPath();
        ctx.moveTo(bx, by - 7 * scale);
        ctx.lineTo(bx, by - 15 * scale);
        ctx.stroke();
        ...
        for (let i = 0; i < 3; i++) {
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx, by + i * 2 * scale);
            ctx.lineTo(bx + dir * 6 * scale, by + (4 + i * 2) * scale);
            ctx.lineTo(bx + dir * 5 * scale, by + (8 + i * 2) * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "centipede": {
        ...
        for (let i = 0; i < segments; i++) {
          const sx = bx + Math.sin(t * 3 + i * 0.8) * 2 * scale;
          const sy = by + (i - segments / 2) * 2.5 * scale;
          const segColor = i === 0 ? "#991b1b" : centColor;
          ctx.fillStyle = segColor;
          ctx.beginPath();
          ctx.ellipse(sx, sy, (3 - i * 0.15) * scale, 1.3 * scale, 0, 0, Math.PI * 2);
          ctx.fill();

          // Legs on each segment (except head)
          if (i > 0) {
            ctx.strokeStyle = "#9a3412";
            ctx.lineWidth = 0.8 * scale;
            const legWave = Math.sin(t * 5 + i * 1.2);
            for (const dir of [-1, 1]) {
              ctx.beginPath();
              ctx.moveTo(sx + dir * 2.5 * scale, sy);
              ctx.lineTo(sx + dir * (5 + legWave) * scale, sy + 2 * scale);
              ctx.stroke();
            }
          }
        }
        ...
        const headX = bx + Math.sin(t * 3) * 2 * scale;
        const headY = by - segments / 2 * 2.5 * scale;
        ctx.strokeStyle = "#dc2626";
        ctx.lineWidth = 1.2 * scale;
        const pincerOpen = Math.sin(t * 4) * 0.3;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(headX + dir * 1.5 * scale, headY);
          ctx.lineTo(headX + dir * (3 + pincerOpen) * scale, headY - 3 * scale);
          ctx.stroke();
        }
        ...
        ctx.fillStyle = "#fbbf24";
        ctx.beginPath();
        ctx.arc(headX - 1.2 * scale, headY - 0.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(headX + 1.2 * scale, headY - 0.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Poison drip from forcipules
        const dripAlpha = 0.4 + Math.sin(t * 6) * 0.3;
        ctx.fillStyle = `rgba(74, 222, 128, ${dripAlpha})`;
        ctx.beginPath();
        ctx.arc(headX, headY - 4 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        break;
      }

      case "dragonfly": {
        const dfColor = "#0ea5e9";
        const bx = cx;
        const by = cy + bounce;
        const wingBeat = Math.sin(t * 10) * 0.4;

        // 4 iridescent wings
        for (let pair = 0; pair < 2; pair++) {
          const wy = by + (-2 + pair * 3) * scale;
          for (const dir of [-1, 1]) {
            const iridGrad = ctx.createLinearGradient(bx, wy, bx + dir * 10 * scale, wy);
            iridGrad.addColorStop(0, "rgba(14, 165, 233, 0.35)");
            iridGrad.addColorStop(0.5, "rgba(168, 85, 247, 0.25)");
            iridGrad.addColorStop(1, "rgba(20, 184, 166, 0.15)");
            ctx.fillStyle = iridGrad;
            ctx.beginPath();
            ctx.ellipse(bx + dir * 6 * scale, wy, (8 - pair) * scale, (2.5 - pair * 0.3) * scale, dir * wingBeat * 0.3, 0, Math.PI * 2);
            ctx.fill();
            // Wing veins
            ...
            ctx.beginPath();
            ctx.moveTo(bx, wy);
            ctx.lineTo(bx + dir * (12 - pair * 2) * scale, wy);
            ctx.stroke();
          }
        }
        ...
        for (let i = 0; i < 6; i++) {
          ctx.fillStyle = i % 2 === 0 ? dfColor : "#0284c7";
          ctx.beginPath();
          ctx.ellipse(bx, by + (3 + i * 2.2) * scale, (2.2 - i * 0.15) * scale, 1.2 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, 3 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.shadowBlur = 4 * scale;
        ctx.beginPath();
        ctx.arc(bx - 2.5 * scale, by - 4 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 2.5 * scale, by - 4 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        // Eye highlights
        ctx.fillStyle = "#fff";
        ctx.beginPath();
        ctx.arc(bx - 3 * scale, by - 4.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 2 * scale, by - 4.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.fill();
        break;
      }

      case "silk_moth": {
        const mothColor = "#c4b5fd";
        const bx = cx;
        const by = cy + bounce;
        const wingPulse = Math.sin(t * 3) * 0.15;
        ...
        for (const dir of [-1, 1]) {
          ...
          ctx.beginPath();
          ctx.ellipse(bx + dir * 6 * scale, by - 2 * scale, 6 * scale, 8 * scale, dir * (0.2 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
          // Wing border
          ...
          ctx.fillStyle = "#7c3aed";
          ctx.beginPath();
          ctx.arc(bx + dir * 6 * scale, by - 2 * scale, 2.5 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "#1e1b4b";
          ctx.beginPath();
          ctx.arc(bx + dir * 6 * scale, by - 2 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "rgba(255,255,255,0.5)";
          ctx.beginPath();
          ctx.arc(bx + dir * 5.5 * scale, by - 2.5 * scale, 0.5 * scale, 0, Math.PI * 2);
          ctx.fill();
          // Lower wings
          ctx.fillStyle = "#ddd6fe";
          ctx.beginPath();
          ctx.ellipse(bx + dir * 4 * scale, by + 5 * scale, 4 * scale, 5 * scale, dir * (0.4 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, 2 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx, by - 6 * scale);
          ctx.quadraticCurveTo(bx + dir * 4 * scale, by - 12 * scale, bx + dir * 6 * scale, by - 14 * scale);
          ctx.stroke();
          // Feather branches
          for (let f = 0; f < 3; f++) {
            const fx = bx + dir * (2 + f * 1.5) * scale;
            const fy = by - (8 + f * 2) * scale;
            ctx.beginPath();
            ctx.moveTo(fx, fy);
            ctx.lineTo(fx + dir * 2 * scale, fy - 1.5 * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "ant_soldier": {
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 6 * scale, 4 * scale, 4.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Petiole (waist)
        ctx.fillStyle = "#78350f";
        ctx.beginPath();
        ctx.arc(bx, by + 1 * scale, 1.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Mesosoma (thorax)
        ctx.fillStyle = antColor;
        ctx.beginPath();
        ctx.ellipse(bx, by - 3 * scale, 3 * scale, 3.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#a0522d";
        ctx.beginPath();
        ctx.arc(bx, by - 8 * scale, 3.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Oversized mandibles
        ctx.strokeStyle = "#451a03";
        ctx.lineWidth = 2 * scale;
        const mandibleOpen = 0.2 + Math.sin(t * 4) * 0.15;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 2 * scale, by - 9 * scale, 3 * scale, Math.PI * 0.5 - dir * mandibleOpen, Math.PI * 0.5 + 0.6 * dir);
          ctx.stroke();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 2 * scale, by - 10 * scale);
          ctx.lineTo(bx + dir * 5 * scale, by - 11 * scale);
          ctx.lineTo(bx + dir * 4 * scale, by - 14 * scale);
          ctx.stroke();
        }
        ...
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(bx - 1.8 * scale, by - 8.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.8 * scale, by - 8.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 3; i++) {
          for (const dir of [-1, 1]) {
            const ly = by + (-4 + i * 3) * scale;
            const legAnim = Math.sin(t * 4 + i * 1.5) * 1.5 * scale;
            ctx.beginPath();
            ctx.moveTo(bx + dir * 2.5 * scale, ly);
            ctx.lineTo(bx + dir * 6 * scale, ly + 2 * scale + legAnim);
            ctx.lineTo(bx + dir * 5 * scale, ly + 5 * scale + legAnim);
            ctx.stroke();
          }
        }
        break;
      ...
      case "locust": {
        const locustColor = "#a3a323";
        const bx = cx;
        const by = cy + bounce;
        const wingSpread = 0.3 + Math.sin(t * 6) * 0.15;

        // Spread wings
        ctx.fillStyle = "rgba(163, 163, 35, 0.3)";
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 6 * scale, by - 2 * scale, 7 * scale, 3.5 * scale, dir * wingSpread, 0, Math.PI * 2);
          ctx.fill();
          // Wing veins
          ctx.strokeStyle = "rgba(163, 163, 35, 0.5)";
          ctx.lineWidth = 0.4 * scale;
          for (let v = 0; v < 4; v++) {
            ctx.beginPath();
            ctx.moveTo(bx, by - 2 * scale);
            ctx.lineTo(bx + dir * (4 + v * 2) * scale, by + (-4 + v * 1.5) * scale);
            ctx.stroke();
          }
        }

        // Abdomen
        ctx.fillStyle = locustColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 3 * scale, 3 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Thorax
        ctx.fillStyle = "#858517";
        ctx.beginPath();
        ctx.ellipse(bx, by - 3 * scale, 3.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = locustColor;
        ctx.beginPath();
        ctx.arc(bx, by - 7 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        // Eyes
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(bx - 1.5 * scale, by - 7.5 * scale, 0.9 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.5 * scale, by - 7.5 * scale, 0.9 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Large hind legs
        ctx.strokeStyle = "#6b6b12";
        ctx.lineWidth = 1.8 * scale;
        for (const dir of [-1, 1]) {
          // Femur (thick upper)
          ctx.beginPath();
          ctx.moveTo(bx + dir * 2 * scale, by + 2 * scale);
          ctx.lineTo(bx + dir * 7 * scale, by - 3 * scale);
          ctx.stroke();
          // Tibia (thin lower)
          ctx.lineWidth = 1 * scale;
          ctx.beginPath();
          ctx.moveTo(bx + dir * 7 * scale, by - 3 * scale);
          ctx.lineTo(bx + dir * 5 * scale, by + 8 * scale);
          ctx.stroke();
          ctx.lineWidth = 1.8 * scale;
        }
        ...
        for (let i = 0; i < 2; i++) {
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx + dir * 2 * scale, by + (-2 + i * 2) * scale);
            ctx.lineTo(bx + dir * 5 * scale, by + (2 + i * 2) * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "trapdoor_spider": {
        const tdColor = "#78350f";
        const bx = cx;
        const by = cy + bounce;

        // Dirt camouflage particles on body
        for (let i = 0; i < 6; i++) {
          const dx = bx + (Math.sin(i * 2.3) * 4) * scale;
          const dy = by + (Math.cos(i * 1.7) * 4) * scale;
          ctx.fillStyle = `rgba(120, 80, 40, ${0.3 + Math.random() * 0.2})`;
          ctx.beginPath();
          ctx.arc(dx, dy, (0.5 + Math.random() * 0.8) * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Stocky legs (shorter, thicker)
        ctx.strokeStyle = "#5c2a0a";
        ctx.lineWidth = 2 * scale;
        for (let i = 0; i < 4; i++) {
          const angle = (i * 0.4 + 0.2);
          for (const dir of [-1, 1]) {
            const kx = bx + dir * 5 * scale * Math.cos(angle);
            const ky = by + 4 * scale * Math.sin(angle) + Math.sin(t * 2 + i) * scale;
            ctx.beginPath();
            ctx.moveTo(bx, by);
            ctx.lineTo(kx, ky);
            ctx.lineTo(kx + dir * 2 * scale, ky + 3 * scale);
            ctx.stroke();
          }
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 5.5 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Dirt texture
        ctx.fillStyle = "#6b3410";
        ctx.beginPath();
        ctx.ellipse(bx - 2 * scale, by + 1 * scale, 1.5 * scale, 1 * scale, 0.3, 0, Math.PI * 2);
        ctx.ellipse(bx + 1.5 * scale, by + 3 * scale, 1 * scale, 1.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();

        // Cephalothorax
        ctx.fillStyle = "#5c2a0a";
        ctx.beginPath();
        ctx.ellipse(bx, by - 4 * scale, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Large downward fangs
        ctx.fillStyle = "#1c1917";
        ctx.beginPath();
        ctx.moveTo(bx - 1.5 * scale, by - 5 * scale);
        ctx.lineTo(bx - 2 * scale, by - 1 * scale);
        ctx.lineTo(bx - 0.5 * scale, by - 4 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(bx + 1.5 * scale, by - 5 * scale);
        ...
        for (let i = 0; i < 4; i++) {
          ctx.beginPath();
          ctx.arc(bx + (i - 1.5) * 1.5 * scale, by - 6 * scale, 0.6 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      }
      ...
      case "ice_beetle": {
        ...
        const frostAlpha = 0.15 + Math.sin(t * 2) * 0.08;
        ctx.fillStyle = `rgba(103, 232, 249, ${frostAlpha})`;
        ctx.beginPath();
        ...
        // Crystalline elytra
        const iceGrad = ctx.createLinearGradient(bx - 5 * scale, by - 5 * scale, bx + 5 * scale, by + 5 * scale);
        iceGrad.addColorStop(0, "#a5f3fc");
        iceGrad.addColorStop(0.5, "#67e8f9");
        iceGrad.addColorStop(1, "#22d3ee");
        ctx.fillStyle = iceGrad;
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 5.5 * scale, 6.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Crystal facets
        ctx.strokeStyle = "rgba(255,255,255,0.4)";
        ctx.lineWidth = 0.6 * scale;
        ...
        ctx.lineTo(bx + 4 * scale, by);
        ctx.moveTo(bx - 3 * scale, by - 4 * scale);
        ctx.lineTo(bx + 3 * scale, by + 4 * scale);
        ctx.stroke();
        ...
        ctx.fillStyle = "#22d3ee";
        ctx.beginPath();
        ctx.ellipse(bx, by - 6 * scale, 3 * scale, 2.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Eyes
        ctx.fillStyle = "#fff";
        ctx.beginPath();
        ctx.arc(bx - 1.5 * scale, by - 6.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.5 * scale, by - 6.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 3; i++) {
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx + dir * 4 * scale, by + (-2 + i * 3) * scale);
            ctx.lineTo(bx + dir * 8 * scale, by + (1 + i * 3) * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "frost_tick": {
        const tickColor = "#a5f3fc";
        const bx = cx;
        const by = cy + bounce;

        // Shield-like flat body
        const shieldGrad = ctx.createRadialGradient(bx, by, 0, bx, by, 7 * scale);
        shieldGrad.addColorStop(0, "#e0f2fe");
        shieldGrad.addColorStop(0.6, tickColor);
        shieldGrad.addColorStop(1, "#67e8f9");
        ctx.fillStyle = shieldGrad;
        ctx.beginPath();
        ctx.ellipse(bx, by, 6 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 1; i <= 3; i++) {
          ctx.beginPath();
          ctx.ellipse(bx, by, i * 2 * scale, i * 2.3 * scale, 0, 0, Math.PI * 2);
          ctx.stroke();
        }
        ...
        ctx.beginPath();
        ctx.moveTo(bx, by - 7 * scale);
        ctx.lineTo(bx, by - 12 * scale);
        ctx.stroke();
        // Barbs
        ctx.lineWidth = 0.6 * scale;
        for (let b = 0; b < 3; b++) {
          const barbY = by - (8 + b * 1.5) * scale;
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx, barbY);
            ctx.lineTo(bx + dir * 1.5 * scale, barbY + 1 * scale);
            ctx.stroke();
          }
        }
        ...
        for (let i = 0; i < 4; i++) {
          for (const dir of [-1, 1]) {
            const ly = by + (-3 + i * 2.5) * scale;
            ctx.beginPath();
            ctx.moveTo(bx + dir * 5 * scale, ly);
            ctx.lineTo(bx + dir * 8 * scale, ly + 2 * scale);
            ctx.stroke();
          }
        }
        ...
        for (let i = 0; i < 4; i++) {
          const px = bx + Math.sin(t * 2 + i * 1.5) * 8 * scale;
          const py = by + Math.cos(t * 2 + i * 1.5) * 8 * scale;
          ctx.beginPath();
      ...
      case "snow_moth": {
        ...
        const bx = cx;
        const by = cy + bounce;
        const wingPulse = Math.sin(t * 3.5) * 0.12;

        // Frost-patterned wings
        for (const dir of [-1, 1]) {
          ctx.fillStyle = snowColor;
          ctx.beginPath();
          ctx.ellipse(bx + dir * 6 * scale, by - 1 * scale, 6 * scale, 8 * scale, dir * (0.15 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
          // Frost pattern
          ctx.strokeStyle = "rgba(186, 230, 253, 0.6)";
          ctx.lineWidth = 0.6 * scale;
          ctx.beginPath();
          ctx.moveTo(bx + dir * 3 * scale, by - 5 * scale);
          ctx.lineTo(bx + dir * 8 * scale, by + 2 * scale);
          ctx.moveTo(bx + dir * 4 * scale, by - 2 * scale);
          ctx.lineTo(bx + dir * 9 * scale, by - 4 * scale);
          ctx.stroke();
          // Lower wings
          ctx.fillStyle = "#f0f9ff";
          ctx.beginPath();
          ctx.ellipse(bx + dir * 4 * scale, by + 5 * scale, 4 * scale, 4.5 * scale, dir * (0.3 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#bae6fd";
        ctx.beginPath();
        ctx.ellipse(bx, by, 1.8 * scale, 5.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx, by - 5.5 * scale);
          ctx.quadraticCurveTo(bx + dir * 3 * scale, by - 10 * scale, bx + dir * 4 * scale, by - 12 * scale);
          ctx.stroke();
        }
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const dist = (8 + Math.sin(t + i) * 3) * scale;
      ...
      case "fire_ant": {
        ...
        ctx.shadowBlur = 8 * scale;
        const glowGrad = ctx.createRadialGradient(bx, by + 6 * scale, 0, bx, by + 6 * scale, 5 * scale);
        glowGrad.addColorStop(0, "#fbbf24");
        glowGrad.addColorStop(0.5, fireAntColor);
        glowGrad.addColorStop(1, "#991b1b");
        ctx.fillStyle = glowGrad;
        ctx.beginPath();
        ctx.ellipse(bx, by + 6 * scale, 4.5 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        ...
        ctx.fillStyle = "#ef4444";
        ctx.beginPath();
        ctx.arc(bx, by - 8 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineTo(bx - 2 * scale, by - 13 * scale);
        ctx.lineTo(bx - 0.5 * scale, by - 11 * scale);
        ctx.lineTo(bx, by - 14 * scale);
        ctx.lineTo(bx + 0.5 * scale, by - 11 * scale);
        ctx.lineTo(bx + 2 * scale, by - 13 * scale);
        ctx.lineTo(bx + 3 * scale, by - 10 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(bx - 1.5 * scale, by - 8.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.5 * scale, by - 8.5 * scale, 0.7 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Antennae
        ctx.strokeStyle = fireAntColor;
        ctx.lineWidth = 0.7 * scale;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 2 * scale, by - 10 * scale);
          ctx.lineTo(bx + dir * 5 * scale, by - 11 * scale);
          ctx.lineTo(bx + dir * 4 * scale, by - 13 * scale);
          ctx.stroke();
        }
        ...
        for (let i = 0; i < 3; i++) {
          for (const dir of [-1, 1]) {
            ctx.beginPath();
            ctx.moveTo(bx + dir * 2 * scale, by + (-3 + i * 3) * scale);
            ctx.lineTo(bx + dir * 6 * scale, by + (0 + i * 3) * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "magma_beetle": {
        const magmaBase = "#b91c1c";
        const bx = cx;
        const by = cy + bounce;

        // Ember particles
        for (let i = 0; i < 4; i++) {
          const ex = bx + Math.sin(t * 3 + i * 1.7) * 8 * scale;
          const ey = by - (3 + i * 2 + Math.sin(t * 2 + i) * 2) * scale;
          const eAlpha = 0.4 + Math.sin(t * 4 + i) * 0.3;
          ctx.fillStyle = `rgba(251, 191, 36, ${eAlpha})`;
          ctx.beginPath();
          ctx.arc(ex, ey, (0.5 + Math.sin(t * 5 + i) * 0.3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Dark obsidian elytra
        ctx.fillStyle = "#1c1917";
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 6 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Lava cracks
        ctx.strokeStyle = magmaBase;
        ctx.lineWidth = 1 * scale;
        ctx.shadowColor = "#f97316";
        ctx.shadowBlur = 4 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 2 * scale, by - 4 * scale);
        ctx.lineTo(bx - 1 * scale, by);
        ctx.lineTo(bx - 3 * scale, by + 4 * scale);
        ctx.moveTo(bx + 1 * scale, by - 3 * scale);
        ctx.lineTo(bx + 2 * scale, by + 1 * scale);
        ctx.lineTo(bx, by + 5 * scale);
        ctx.moveTo(bx - 1 * scale, by);
        ctx.lineTo(bx + 2 * scale, by + 1 * scale);
        ctx.stroke();
        ctx.shadowBlur = 0;

        // Lava glow through cracks
        const lavaGlow = 0.3 + Math.sin(t * 3) * 0.15;
        ctx.fillStyle = `rgba(249, 115, 22, ${lavaGlow})`;
        ctx.beginPath();
        ctx.ellipse(bx, by, 3 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.shadowBlur = 5 * scale;
        ctx.beginPath();
        ctx.arc(bx - 1.5 * scale, by - 7.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.5 * scale, by - 7.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
      ...
      case "ash_moth": {
        const ashColor = "#f97316";
        const bx = cx;
        const by = cy + bounce;
        const wingPulse = Math.sin(t * 4) * 0.18;

        // Ember trail behind
        for (let i = 0; i < 5; i++) {
          const tx = bx + Math.sin(t * 2 + i * 1.3) * 3 * scale;
          const ty = by + (8 + i * 2.5) * scale;
          const tAlpha = 0.5 - i * 0.09;
          ctx.fillStyle = `rgba(249, 115, 22, ${tAlpha})`;
          ctx.beginPath();
          ctx.arc(tx, ty, (1.2 - i * 0.15) * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Fiery wings
        for (const dir of [-1, 1]) {
          const wingGrad = ctx.createRadialGradient(bx + dir * 5 * scale, by, 0, bx + dir * 5 * scale, by, 8 * scale);
          wingGrad.addColorStop(0, "#fbbf24");
          wingGrad.addColorStop(0.5, ashColor);
          wingGrad.addColorStop(1, "#dc2626");
          ctx.fillStyle = wingGrad;
          ctx.beginPath();
          ctx.ellipse(bx + dir * 5.5 * scale, by - 1 * scale, 5.5 * scale, 7 * scale, dir * (0.2 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
          // Hot edge glow
          ...
          ctx.fillStyle = "#ea580c";
          ctx.beginPath();
          ctx.ellipse(bx + dir * 3.5 * scale, by + 5 * scale, 3.5 * scale, 4 * scale, dir * (0.35 + wingPulse), 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#7c2d12";
        ctx.beginPath();
        ctx.ellipse(bx, by, 1.8 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx, by - 5 * scale);
          ctx.quadraticCurveTo(bx + dir * 3 * scale, by - 10 * scale, bx + dir * 5 * scale, by - 11 * scale);
          ctx.stroke();
        }
        ...
        ctx.shadowBlur = 3 * scale;
        ctx.beginPath();
        ctx.arc(bx - 1 * scale, by - 4.5 * scale, 0.6 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1 * scale, by - 4.5 * scale, 0.6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        break;
      ...
      case "brood_mother": {
        ...
        const auraAlpha = 0.1 + Math.sin(t * 1.5) * 0.06;
        ctx.fillStyle = `rgba(220, 38, 38, ${auraAlpha})`;
        ctx.beginPath();
        ctx.arc(bx, by, 14 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;

        // Silk threads radiating outward
        ctx.strokeStyle = "rgba(255,255,255,0.1)";
        ctx.lineWidth = 0.4 * scale;
        for (let i = 0; i < 8; i++) {
          const angle = (i / 8) * Math.PI * 2 + t * 0.3;
          ctx.beginPath();
          ctx.moveTo(bx + Math.cos(angle) * 8 * scale, by + Math.sin(angle) * 8 * scale);
          ctx.lineTo(bx + Math.cos(angle) * 16 * scale, by + Math.sin(angle) * 16 * scale);
          ctx.stroke();
        }
        ...
        ctx.lineWidth = 2.5 * scale;
        for (let i = 0; i < 4; i++) {
          const angle = (i * 0.35 + 0.25);
          const legLen = (10 + i * 2) * scale;
          const kneeLen = legLen * 0.5;
          const legWave = Math.sin(t * 2 + i * 1.2) * 2 * scale;
          for (const dir of [-1, 1]) {
            const kx = bx + dir * kneeLen * Math.cos(angle);
            const ky = by + kneeLen * Math.sin(angle) + legWave;
            const ex = bx + dir * legLen * Math.cos(angle - 0.25);
            const ey = by + legLen * Math.sin(angle + 0.15) + legWave * 0.6;
            ctx.beginPath();
            ctx.moveTo(bx, by);
            ctx.lineTo(kx, ky);
            ctx.lineTo(ex, ey);
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 3 * scale, 7 * scale, 8 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        // Red hourglass marking
        ctx.fillStyle = "#dc2626";
        ctx.beginPath();
        ctx.moveTo(bx, by + 1 * scale);
        ctx.lineTo(bx - 2.5 * scale, by - 2 * scale);
        ctx.lineTo(bx + 2.5 * scale, by - 2 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(bx, by + 1 * scale);
        ctx.lineTo(bx - 2.5 * scale, by + 4 * scale);
        ctx.lineTo(bx + 2.5 * scale, by + 4 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.fillStyle = "#292524";
        ctx.beginPath();
        ctx.ellipse(bx, by - 6 * scale, 5 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // 8 red eyes (2 rows of 4)
        ctx.fillStyle = "#dc2626";
        ctx.shadowColor = "#dc2626";
        ctx.shadowBlur = 5 * scale;
        for (let row = 0; row < 2; row++) {
          for (let i = 0; i < 4; i++) {
            const ex = bx + (i - 1.5) * 1.8 * scale;
            const ey = by - (7 + row * 1.8) * scale;
            const er = (row === 0 ? 0.7 : 0.5) * scale;
            ctx.beginPath();
            ctx.arc(ex, ey, er, 0, Math.PI * 2);
            ctx.fill();
          }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 1.5 * scale, by - 5 * scale);
          ctx.lineTo(bx + dir * 2.5 * scale, by - 1 * scale);
          ctx.lineTo(bx + dir * 0.5 * scale, by - 4 * scale);
          ctx.closePath();
          ctx.fill();
        }
        break;
      ...
      case "dire_bear": {
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, 10 * scale, 8 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Layered fur shading
        ctx.fillStyle = "#4a2e15";
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 8 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#6b4423";
        ctx.beginPath();
        ctx.arc(bx, by - 9 * scale, 6 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.fillStyle = bearColor;
          ctx.beginPath();
          ctx.arc(bx + dir * 5 * scale, by - 13 * scale, 2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 7 * scale, 3 * scale, 2 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 2.5 * scale, by - 10 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        ctx.lineWidth = 0.8 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 4 * scale, by - 11 * scale);
        ctx.lineTo(bx - 1 * scale, by - 8 * scale);
        ctx.stroke();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 7 * scale, by + 7 * scale, 3 * scale, 2 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      }
      ...
      case "ancient_ent": {
        ...
        ctx.lineTo(bx - 4 * scale, by - 8 * scale);
        ctx.lineTo(bx + 4 * scale, by - 8 * scale);
        ctx.lineTo(bx + 6 * scale, by + 10 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        for (let i = -2; i <= 2; i++) {
          ctx.beginPath();
          ctx.moveTo(bx + i * 2 * scale, by - 6 * scale);
          ctx.lineTo(bx + i * 2.5 * scale, by + 8 * scale);
          ctx.stroke();
        }

        // Glowing green heartwood
        const heartGlow = 0.5 + Math.sin(t * 2) * 0.3;
        ctx.fillStyle = `rgba(80, 220, 60, ${heartGlow})`;
        ctx.shadowColor = "#50dc3c";
        ctx.shadowBlur = 8 * scale;
        ctx.beginPath();
        ctx.ellipse(bx, by, 2.5 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 3 * scale, by - 4 * scale);
          ctx.lineTo(bx + dir * 10 * scale, by - 8 * scale);
          ctx.lineTo(bx + dir * 13 * scale, by - 12 * scale);
          ctx.stroke();
        }
        ...
        for (const dir of [-1, 1]) {
          for (let i = 0; i < 3; i++) {
            ctx.beginPath();
            ctx.arc(bx + dir * (10 + i * 2) * scale, by - (10 + i * 1.5) * scale, 2.5 * scale, 0, Math.PI * 2);
            ctx.fill();
          }
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 2 * scale, by - 5 * scale, 1.2 * scale, 1.8 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }

        // Mouth crevice
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale, 1.5 * scale, 0.8 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        break;
      }

      case "forest_troll": {
        const trollColor = "#4a6741";
        const bx = cx;
        const by = cy + bounce;
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 8 * scale, 10 * scale, 0.15, 0, Math.PI * 2);
        ctx.fill();

        // Warty bumps
        ctx.fillStyle = "#3a5231";
        for (let i = 0; i < 6; i++) {
          const wx = bx + Math.cos(i * 1.1) * 5 * scale;
          const wy = by + Math.sin(i * 1.3) * 6 * scale;
          ctx.beginPath();
          ctx.arc(wx, wy, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.arc(bx + 2 * scale, by - 9 * scale, 4.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + 2 * scale + dir * 2 * scale, by - 7 * scale);
          ctx.lineTo(bx + 2 * scale + dir * 3 * scale, by - 12 * scale);
          ctx.lineTo(bx + 2 * scale + dir * 1 * scale, by - 8 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 2 * scale + dir * 2 * scale, by - 10 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.fillStyle = "#000";
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 2 * scale + dir * 2 * scale, by - 10 * scale, 0.5 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Tree-trunk club (held in right hand)
        ctx.fillStyle = "#5a3a1e";
        ctx.beginPath();
        ctx.moveTo(bx + 8 * scale, by - 2 * scale);
        ctx.lineTo(bx + 12 * scale, by - 14 * scale);
        ctx.lineTo(bx + 14 * scale, by - 13 * scale);
        ctx.lineTo(bx + 10 * scale, by - 1 * scale);
        ctx.closePath();
        ctx.fill();
        break;
      }
      ...
      case "timber_wolf": {
        const wolfColor = "#6b6b6b";
        const bx = cx;
        const by = cy + bounce;
        const runPhase = Math.sin(t * 4) * 2 * scale;

        // Body (elongated)
        ctx.fillStyle = wolfColor;
        ctx.beginPath();
        ctx.ellipse(bx, by, 10 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Fur texture - lighter belly
        ctx.fillStyle = "#8a8a8a";
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 7 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = wolfColor;
        ctx.beginPath();
        ctx.ellipse(bx + 8 * scale, by - 3 * scale, 4 * scale, 3.5 * scale, -0.3, 0, Math.PI * 2);
        ctx.fill();

        // Snout
        ctx.fillStyle = "#7a7a7a";
        ctx.beginPath();
        ctx.ellipse(bx + 12 * scale, by - 3.5 * scale, 2.5 * scale, 1.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.fillStyle = wolfColor;
          ctx.beginPath();
          ctx.moveTo(bx + 7 * scale + dir * 2 * scale, by - 5 * scale);
          ctx.lineTo(bx + 6 * scale + dir * 2 * scale, by - 10 * scale);
          ctx.lineTo(bx + 9 * scale + dir * 2 * scale, by - 5 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 9 * scale, by - 4 * scale + dir * 1.2 * scale, 0.8 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        ctx.lineWidth = 1.5 * scale;
        for (let i = 0; i < 4; i++) {
          const lx = bx + (i - 1.5) * 4.5 * scale;
          const phase = runPhase * (i % 2 === 0 ? 1 : -1);
          ctx.beginPath();
          ctx.moveTo(lx, by + 4 * scale);
          ctx.lineTo(lx + phase * 0.3, by + 10 * scale + phase);
          ctx.stroke();
        }
        ...
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 9 * scale, by - 1 * scale);
        ctx.quadraticCurveTo(bx - 13 * scale, by - 6 * scale, bx - 11 * scale, by - 9 * scale);
        ctx.stroke();
        break;
      }

      case "giant_eagle": {
        const eagleColor = "#c8a050";
        const bx = cx;
        const by = cy + bounce;
        const wingFlap = Math.sin(t * 3) * 0.3;

        // Wing shadow
        ctx.fillStyle = `${eagleColor}30`;
        ctx.beginPath();
        ctx.ellipse(bx, by, 16 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ...
          ctx.moveTo(bx, by - 2 * scale);
          ctx.quadraticCurveTo(
            bx + dir * 10 * scale, by - (10 + wingFlap * 10) * scale,
            bx + dir * 16 * scale, by - (4 + wingFlap * 8) * scale
          );
          ctx.lineTo(bx + dir * 12 * scale, by + 1 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          for (let i = 0; i < 4; i++) {
            const fx = bx + dir * (6 + i * 2.5) * scale;
            const fy = by - (2 + wingFlap * 6 - i * 0.5) * scale;
            ctx.beginPath();
            ctx.ellipse(fx, fy, 2 * scale, 1 * scale, dir * 0.5, 0, Math.PI * 2);
            ctx.fill();
          }
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, 4 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#f0e8d0";
        ctx.beginPath();
        ctx.arc(bx, by - 7 * scale, 3 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(bx, by - 7 * scale);
        ctx.lineTo(bx + 4 * scale, by - 6 * scale);
        ctx.lineTo(bx, by - 5 * scale);
        ctx.closePath();
        ctx.fill();
        ...
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(bx - 1 * scale, by - 7.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          for (let i = 0; i < 3; i++) {
            ctx.beginPath();
            ctx.moveTo(bx + dir * 2 * scale, by + 5 * scale);
            ctx.lineTo(bx + dir * (2 + i - 1) * scale, by + 8 * scale);
            ctx.stroke();
          }
        }
        break;
      ...
      case "swamp_hydra": {
        const hydraColor = "#2e4a2e";
        const bx = cx;
        const by = cy + bounce;

        // Scaled body
        ctx.fillStyle = hydraColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 4 * scale, 8 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let row = 0; row < 3; row++) {
          for (let i = 0; i < 4; i++) {
            ctx.beginPath();
            ctx.arc(bx + (i - 1.5) * 3 * scale, by + (2 + row * 2.5) * scale, 1.5 * scale, Math.PI, 0);
            ctx.fill();
          }
        }

        // Three snake necks
        const neckOffsets = [-4, 0, 4];
        const neckAngles = [-0.4, 0, 0.4];
        for (let n = 0; n < 3; n++) {
          const sway = Math.sin(t * 2 + n * 2) * 2 * scale;
          const nx = bx + neckOffsets[n] * scale + sway;
          const ny = by - 6 * scale;
          ...
          ctx.lineWidth = 3 * scale;
          ctx.beginPath();
          ctx.moveTo(bx + neckOffsets[n] * 0.5 * scale, by);
          ctx.quadraticCurveTo(nx + neckAngles[n] * 4 * scale, ny + 4 * scale, nx, ny - 4 * scale);
          ctx.stroke();

          // Head
          ctx.fillStyle = "#3a5a3a";
          ctx.beginPath();
          ctx.ellipse(nx, ny - 6 * scale, 2.5 * scale, 2 * scale, neckAngles[n], 0, Math.PI * 2);
          ctx.fill();

          // Eyes
          ctx.fillStyle = "#ff4444";
          ctx.beginPath();
          ctx.arc(nx - 1 * scale, ny - 7 * scale, 0.6 * scale, 0, Math.PI * 2);
          ctx.fill();

          // Dripping venom
          ctx.fillStyle = "#44ff44";
          ctx.globalAlpha = 0.6;
          const dripY = (t * 20 + n * 10) % (8 * scale);
          ctx.beginPath();
          ctx.arc(nx, ny - 4 * scale + dripY, 0.5 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.globalAlpha = 1.0;
        }
        break;
      ...
      case "giant_toad": {
        const toadColor = "#5a7a3a";
        const bx = cx;
        const by = cy + bounce;
        const breathe = Math.sin(t * 1.5) * 1.5 * scale;

        // Bloated body
        ctx.fillStyle = toadColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 10 * scale + breathe, 8 * scale + breathe * 0.5, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const wx = bx + Math.cos(angle) * 7 * scale;
          const wy = by + 2 * scale + Math.sin(angle) * 5 * scale;
          ctx.beginPath();
          ctx.arc(wx, wy, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 5 * scale, 6 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Bulging eyes (on top of head)
        for (const dir of [-1, 1]) {
          ctx.fillStyle = "#dde830";
          ctx.beginPath();
          ctx.arc(bx + dir * 4 * scale, by - 6 * scale, 3 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "#000";
          ctx.beginPath();
          ctx.arc(bx + dir * 4 * scale, by - 6 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // Throat sac (inflating)
        const sacSize = 3 + Math.sin(t * 2) * 2;
        ctx.fillStyle = `rgba(180, 220, 120, 0.6)`;
        ctx.beginPath();
        ctx.ellipse(bx, by + 8 * scale, sacSize * scale, sacSize * 0.7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 6 * scale, by - 2 * scale);
        ctx.quadraticCurveTo(bx, by, bx + 6 * scale, by - 2 * scale);
        ctx.stroke();
        break;
      }

      case "vine_serpent": {
        const vineColor = "#3a6b3a";
        const bx = cx;
        const by = cy + bounce;
        ...
        ctx.lineWidth = 3 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 10 * scale, by + 8 * scale);
        for (let i = 0; i < 8; i++) {
          const px = bx - 10 * scale + i * 2.8 * scale;
          const py = by + Math.sin(t * 2 + i * 0.8) * 3 * scale;
          ctx.lineTo(px, py);
        }
        ctx.stroke();
        ...
        for (let i = 1; i < 7; i++) {
          const tx = bx - 10 * scale + i * 2.8 * scale;
          const ty = by + Math.sin(t * 2 + i * 0.8) * 3 * scale;
          const side = i % 2 === 0 ? -1 : 1;
          ctx.beginPath();
          ctx.moveTo(tx, ty);
          ctx.lineTo(tx + side * 2 * scale, ty - 2 * scale);
          ctx.lineTo(tx + 0.5 * scale, ty);
          ctx.closePath();
          ctx.fill();
        }
        ...
        for (let i = 0; i < 3; i++) {
          const lx = bx - 6 * scale + i * 5 * scale;
          const ly = by + Math.sin(t * 2 + i * 1.6) * 3 * scale;
          ctx.beginPath();
          ctx.ellipse(lx, ly - 2 * scale, 1.5 * scale, 3 * scale, Math.PI * 0.3, 0, Math.PI * 2);
          ctx.fill();
        }

        // Flower head
        const hx = bx + 10 * scale;
        const hy = by + Math.sin(t * 2 + 6) * 3 * scale;
        ctx.fillStyle = "#ff6688";
        for (let p = 0; p < 5; p++) {
          const pa = (p / 5) * Math.PI * 2 + t * 0.5;
          ctx.beginPath();
          ctx.ellipse(
            hx + Math.cos(pa) * 2 * scale, hy + Math.sin(pa) * 2 * scale,
            2 * scale, 1 * scale, pa, 0, Math.PI * 2
          );
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#000";
        ctx.beginPath();
        ctx.arc(hx, hy, 0.6 * scale, 0, Math.PI * 2);
        ctx.fill();
        break;
      }

      case "marsh_troll": {
        const mTrollColor = "#4a5a3a";
        const bx = cx;
        const by = cy + bounce;

        // Mud-covered body
        ctx.fillStyle = mTrollColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 8 * scale, 10 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Mud drip streaks
        ctx.fillStyle = "#3a4a2a";
        for (let i = 0; i < 5; i++) {
          const mx = bx + (i - 2) * 3 * scale;
          ctx.beginPath();
          ctx.moveTo(mx, by - 4 * scale);
          ctx.lineTo(mx - 0.5 * scale, by + 8 * scale);
          ctx.lineTo(mx + 0.5 * scale, by + 8 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.arc(bx, by - 9 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Glowing fungus on shoulders
        ctx.shadowColor = "#88ff88";
        ctx.shadowBlur = 6 * scale;
        const fungusGlow = 0.5 + Math.sin(t * 3) * 0.3;
        ctx.fillStyle = `rgba(120, 255, 120, ${fungusGlow})`;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 7 * scale, by - 5 * scale, 2 * scale, 0, Math.PI * 2);
          ctx.fill();
          ctx.beginPath();
          ctx.arc(bx + dir * 6 * scale, by - 7 * scale, 1.5 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 2 * scale, by - 10 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(bx + 8 * scale, by + 5 * scale);
        ctx.lineTo(bx + 10 * scale, by - 12 * scale);
        ctx.stroke();
        ctx.fillStyle = "#d8d0b8";
        ctx.beginPath();
        ctx.arc(bx + 10 * scale, by - 13 * scale, 2.5 * scale, 0, Math.PI * 2);
        ctx.fill();
        break;
      }
      ...
      case "phoenix": {
        ...
        ctx.shadowBlur = 15 * scale;
        const auraGrad = ctx.createRadialGradient(bx, by, 0, bx, by, 14 * scale);
        auraGrad.addColorStop(0, `rgba(255, 200, 50, ${flamePulse * 0.4})`);
        ...
        // Flame body
        const bodyGrad = ctx.createRadialGradient(bx, by, 0, bx, by, 6 * scale);
        bodyGrad.addColorStop(0, "#ffffa0");
        bodyGrad.addColorStop(0.4, "#ff8800");
        bodyGrad.addColorStop(1, "#cc3300");
        ctx.fillStyle = bodyGrad;
        ctx.beginPath();
        ctx.ellipse(bx, by, 5 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Fire wings
        for (const dir of [-1, 1]) {
          const wingGrad = ctx.createLinearGradient(bx, by, bx + dir * 14 * scale, by - 6 * scale);
          wingGrad.addColorStop(0, "#ffcc00");
          wingGrad.addColorStop(1, "#ff4400");
          ctx.fillStyle = wingGrad;
          ctx.beginPath();
          ctx.moveTo(bx, by - 2 * scale);
          ctx.quadraticCurveTo(
            bx + dir * 8 * scale, by - 12 * scale,
            bx + dir * 14 * scale, by - 6 * scale
          );
          ctx.quadraticCurveTo(
            bx + dir * 10 * scale, by + 2 * scale,
            bx, by + 3 * scale
          );
          ctx.closePath();
          ctx.fill();
        ...
        // Crown flames
        for (let i = 0; i < 5; i++) {
          const fx = bx + (i - 2) * 1.5 * scale;
          const fh = (3 + Math.sin(t * 6 + i) * 2) * scale;
          ctx.fillStyle = i % 2 === 0 ? "#ffaa00" : "#ff6600";
          ctx.beginPath();
          ctx.moveTo(fx - 0.8 * scale, by - 10 * scale);
          ctx.lineTo(fx, by - 10 * scale - fh);
          ctx.lineTo(fx + 0.8 * scale, by - 10 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        ctx.fillStyle = "#ffffff";
        ctx.beginPath();
        ctx.arc(bx - 1.2 * scale, by - 8.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.arc(bx + 1.2 * scale, by - 8.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Ember particles
        for (let i = 0; i < 6; i++) {
          const angle = t * 2 + i * 1.05;
          const dist = (6 + Math.sin(t * 3 + i) * 4) * scale;
          const ex = bx + Math.cos(angle) * dist;
          const ey = by + Math.sin(angle) * dist - 3 * scale;
          ctx.fillStyle = `rgba(255, ${150 + i * 15}, 0, ${0.8 - i * 0.1})`;
          ctx.beginPath();
          ctx.arc(ex, ey, (0.5 + Math.sin(t * 5 + i) * 0.3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      case "basilisk": {
        const basColor = "#6b6b3a";
        const bx = cx;
        const by = cy + bounce;

        // Coiled body
        ctx.fillStyle = basColor;
        ctx.lineWidth = 4 * scale;
        ctx.strokeStyle = basColor;
        ctx.beginPath();
        ctx.arc(bx, by + 4 * scale, 6 * scale, 0, Math.PI * 1.5);
        ctx.stroke();
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const sy = by + 4 * scale + Math.sin(angle) * sr;
          ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 8 * scale, 4 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(bx - 3 * scale, by - 10 * scale);
        ctx.lineTo(bx - 1.5 * scale, by - 15 * scale);
        ctx.lineTo(bx, by - 12 * scale);
        ctx.lineTo(bx + 1.5 * scale, by - 15 * scale);
        ctx.lineTo(bx + 3 * scale, by - 10 * scale);
        ctx.closePath();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 1.8 * scale, by - 8.5 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = 1.0;
        ctx.shadowBlur = 0;
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 1.8 * scale, by - 8.5 * scale, 0.3 * scale, 1 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.lineWidth = 0.5 * scale;
        const tongueExt = Math.sin(t * 4) * 2 * scale;
        ctx.beginPath();
        ctx.moveTo(bx, by - 6 * scale);
        ctx.lineTo(bx, by - 5 * scale + tongueExt);
        ctx.moveTo(bx, by - 5 * scale + tongueExt);
        ctx.lineTo(bx - 1 * scale, by - 4 * scale + tongueExt);
        ctx.moveTo(bx, by - 5 * scale + tongueExt);
        ctx.lineTo(bx + 1 * scale, by - 4 * scale + tongueExt);
        ctx.stroke();
        break;
      ...
      case "djinn": {
        const bx = cx;
        const by = cy + bounce;
        const floatOffset = Math.sin(t * 2) * 2 * scale;

        // Smoky lower body
        ctx.globalAlpha = 0.5;
        const smokeGrad = ctx.createRadialGradient(bx, by + 8 * scale, 0, bx, by + 8 * scale, 8 * scale);
        smokeGrad.addColorStop(0, "#6a3aaa");
        smokeGrad.addColorStop(1, "rgba(106, 58, 170, 0)");
        ctx.fillStyle = smokeGrad;
        ctx.beginPath();
        ctx.moveTo(bx - 6 * scale, by + 2 * scale);
        ctx.quadraticCurveTo(bx - 8 * scale, by + 12 * scale, bx, by + 14 * scale);
        ctx.quadraticCurveTo(bx + 8 * scale, by + 12 * scale, bx + 6 * scale, by + 2 * scale);
        ctx.closePath();
        ctx.fill();
        ctx.globalAlpha = 1.0;
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale + floatOffset, 6 * scale, 7 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 5 * scale, by - 3 * scale + floatOffset);
          ctx.quadraticCurveTo(bx + dir * 9 * scale, by - 6 * scale, bx + dir * 7 * scale, by - 10 * scale);
          ctx.stroke();
        }

        // Head
        ctx.fillStyle = "#9a6adc";
        ctx.beginPath();
        ctx.arc(bx, by - 10 * scale + floatOffset, 3.5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Golden turban
        ctx.fillStyle = "#ffd700";
        ctx.beginPath();
        ctx.arc(bx, by - 12 * scale + floatOffset, 4 * scale, Math.PI, 0);
        ctx.fill();
        ctx.fillStyle = "#ff4444";
        ctx.beginPath();
        ctx.arc(bx, by - 14 * scale + floatOffset, 1.2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 1.5 * scale, by - 10 * scale + floatOffset, 0.8 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        ctx.font = `${3 * scale}px serif`;
        for (let i = 0; i < 4; i++) {
          const runeAngle = t * 1.5 + i * Math.PI * 0.5;
          const rx = bx + Math.cos(runeAngle) * 10 * scale;
          const ry = by - 4 * scale + Math.sin(runeAngle) * 6 * scale + floatOffset;
          ctx.globalAlpha = 0.6 + Math.sin(t * 3 + i) * 0.3;
          ctx.beginPath();
          ctx.arc(rx, ry, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = 1.0;
        break;
      ...
      case "manticore": {
        const mantiColor = "#8b4513";
        const bx = cx;
        const by = cy + bounce;

        // Lion body
        ctx.fillStyle = mantiColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 10 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Flame mane
        const maneGrad = ctx.createRadialGradient(bx - 2 * scale, by - 5 * scale, 0, bx - 2 * scale, by - 5 * scale, 8 * scale);
        maneGrad.addColorStop(0, "#ff8800");
        maneGrad.addColorStop(0.6, "#cc4400");
        maneGrad.addColorStop(1, "#881100");
        ctx.fillStyle = maneGrad;
        ctx.beginPath();
        for (let i = 0; i < 8; i++) {
          const angle = (i / 8) * Math.PI * 2;
          const r = (6 + Math.sin(t * 4 + i * 0.8) * 1.5) * scale;
          const mx = bx - 2 * scale + Math.cos(angle) * r;
          const my = by - 5 * scale + Math.sin(angle) * r;
        ...
        ctx.beginPath();
        ctx.arc(bx - 2 * scale, by - 5 * scale, 4 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx - 2 * scale + dir * 1.5 * scale, by - 6 * scale, 0.8 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 3 * scale, by - 1 * scale);
          ctx.quadraticCurveTo(bx + dir * 12 * scale, by - 10 * scale, bx + dir * 14 * scale, by - 4 * scale);
          ctx.quadraticCurveTo(bx + dir * 11 * scale, by - 2 * scale, bx + dir * 10 * scale, by + 2 * scale);
          ctx.lineTo(bx + dir * 3 * scale, by + 1 * scale);
          ctx.closePath();
          ctx.fill();
        }

        // Scorpion tail curling overhead
        ctx.strokeStyle = "#6b3010";
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(bx + 8 * scale, by + 4 * scale);
        ctx.quadraticCurveTo(bx + 12 * scale, by - 4 * scale, bx + 8 * scale, by - 12 * scale);
        ctx.quadraticCurveTo(bx + 4 * scale, by - 16 * scale, bx + 2 * scale, by - 14 * scale);
        ctx.stroke();
        // Stinger
        ctx.fillStyle = "#222";
        ctx.beginPath();
        ctx.moveTo(bx + 2 * scale, by - 14 * scale);
        ctx.lineTo(bx + 1 * scale, by - 17 * scale);
        ctx.lineTo(bx + 3 * scale, by - 15 * scale);
        ctx.closePath();
        ctx.fill();

        // Legs
        ctx.strokeStyle = mantiColor;
        ctx.lineWidth = 1.5 * scale;
        for (let i = 0; i < 4; i++) {
          const lx = bx + (i - 1.5) * 4 * scale;
          ctx.beginPath();
          ctx.moveTo(lx, by + 7 * scale);
          ctx.lineTo(lx, by + 12 * scale);
          ctx.stroke();
        }
        break;
      }
      ...
      case "frost_troll": {
        const fTrollColor = "#5a8aaa";
        const bx = cx;
        const by = cy + bounce;

        // Bulky body
        ctx.fillStyle = fTrollColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 9 * scale, 11 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const [ox, oy] of crystalPositions) {
          ctx.beginPath();
          ctx.moveTo(bx + ox * scale, by + (oy - 2) * scale);
          ctx.lineTo(bx + (ox + 1) * scale, by + oy * scale);
          ctx.lineTo(bx + ox * scale, by + (oy + 2) * scale);
          ctx.lineTo(bx + (ox - 1) * scale, by + oy * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        ctx.beginPath();
        ctx.arc(bx, by - 10 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();

        // Brow ridge
        ctx.fillStyle = "#4a7a9a";
        ctx.beginPath();
        ctx.ellipse(bx, by - 12 * scale, 5 * scale, 1.5 * scale, 0, 0, Math.PI);
        ctx.fill();
        ...
        for (let i = 0; i < 4; i++) {
          const bDist = (3 + i * 2) * scale;
          ctx.beginPath();
          ctx.arc(bx + bDist, by - 8 * scale + Math.sin(t + i) * scale, (1.5 - i * 0.2) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = 1.0;
        ...
        ctx.beginPath();
        ctx.moveTo(bx - 8 * scale, by + 5 * scale);
        ctx.lineTo(bx - 10 * scale, by - 8 * scale);
        ctx.lineTo(bx - 12 * scale, by - 12 * scale);
        ctx.lineTo(bx - 8 * scale, by - 10 * scale);
      ...
      case "dire_wolf": {
        const dWolfColor = "#c8c8d8";
        const bx = cx;
        const by = cy + bounce;

        // Large body
        ctx.fillStyle = dWolfColor;
        ctx.beginPath();
        ctx.ellipse(bx, by, 11 * scale, 6 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Frost-white fur overlay
        ctx.fillStyle = "#e8e8f0";
        ctx.beginPath();
        ctx.ellipse(bx - 2 * scale, by - 1 * scale, 8 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = dWolfColor;
        ctx.beginPath();
        ctx.ellipse(bx + 9 * scale, by - 3 * scale, 5 * scale, 4 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();

        // Snout
        ctx.fillStyle = "#d8d8e8";
        ctx.beginPath();
        ctx.ellipse(bx + 14 * scale, by - 3 * scale, 3 * scale, 2 * scale, -0.1, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.fillStyle = dWolfColor;
          ctx.beginPath();
          ctx.moveTo(bx + 8 * scale + dir * 2 * scale, by - 6 * scale);
          ctx.lineTo(bx + 7 * scale + dir * 2.5 * scale, by - 12 * scale);
          ctx.lineTo(bx + 10 * scale + dir * 2 * scale, by - 6 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 10 * scale, by - 4 * scale + dir * 1.5 * scale, 1 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        ctx.fillStyle = "#aaddff";
        for (let i = 0; i < 5; i++) {
          const bd = (4 + i * 2.5) * scale;
          ctx.beginPath();
          ctx.arc(bx + 14 * scale + bd, by - 3 * scale + Math.sin(t * 2 + i) * 1.5 * scale, (2 - i * 0.3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = 1.0;

        // Legs
        ctx.strokeStyle = dWolfColor;
        ctx.lineWidth = 1.8 * scale;
        for (let i = 0; i < 4; i++) {
          const lx = bx + (i - 1.5) * 5 * scale;
          ctx.beginPath();
          ctx.moveTo(lx, by + 5 * scale);
          ctx.lineTo(lx, by + 11 * scale);
          ctx.stroke();
        }
        ...
        ctx.beginPath();
        ctx.moveTo(bx - 10 * scale, by - 1 * scale);
        ctx.quadraticCurveTo(bx - 16 * scale, by - 6 * scale, bx - 13 * scale, by - 10 * scale);
        ctx.quadraticCurveTo(bx - 10 * scale, by - 7 * scale, bx - 9 * scale, by - 1 * scale);
        ctx.closePath();
        ctx.fill();
        break;
      }
      ...
      case "wendigo": {
        const wendigoColor = "#3a3a4a";
        const bx = cx;
        const by = cy + bounce;

        // Gaunt skeletal body
        ctx.fillStyle = wendigoColor;
        ctx.beginPath();
        ctx.ellipse(bx, by + 2 * scale, 5 * scale, 10 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 4; i++) {
          ctx.beginPath();
          ctx.ellipse(bx, by - 2 * scale + i * 2.5 * scale, 4 * scale, 0.8 * scale, 0, 0, Math.PI);
          ctx.stroke();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 4 * scale, by - 3 * scale);
          ctx.lineTo(bx + dir * 10 * scale, by + 4 * scale);
          ctx.lineTo(bx + dir * 12 * scale, by + 8 * scale);
          ctx.stroke();
          // Claws
          for (let c = 0; c < 3; c++) {
            ctx.beginPath();
            ctx.moveTo(bx + dir * 12 * scale, by + 8 * scale);
            ctx.lineTo(bx + dir * (13 + c * 0.5) * scale, by + (10 + c * 0.3) * scale);
            ctx.stroke();
          }
        }
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by - 12 * scale, 4 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Skull snout
        ctx.fillStyle = "#c8c0b0";
        ctx.beginPath();
        ctx.ellipse(bx, by - 8 * scale, 2 * scale, 3 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 3 * scale, by - 15 * scale);
          ctx.lineTo(bx + dir * 6 * scale, by - 20 * scale);
          ctx.lineTo(bx + dir * 8 * scale, by - 22 * scale);
          ctx.moveTo(bx + dir * 6 * scale, by - 20 * scale);
          ctx.lineTo(bx + dir * 5 * scale, by - 23 * scale);
          ctx.moveTo(bx + dir * 4 * scale, by - 17 * scale);
          ctx.lineTo(bx + dir * 7 * scale, by - 18 * scale);
          ctx.stroke();
        }
        ...
        ctx.shadowBlur = 8 * scale;
        const eyeFlicker = 0.5 + Math.sin(t * 5) * 0.5;
        ctx.globalAlpha = eyeFlicker;
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 1.5 * scale, by - 13 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.globalAlpha = 1.0;
        ctx.shadowBlur = 0;
      ...
      case "mammoth": {
        const mammothColor = "#8b7355";
        const bx = cx;
        const by = cy + bounce;

        // Massive body with fur
        ctx.fillStyle = mammothColor;
        ctx.beginPath();
        ctx.ellipse(bx, by, 12 * scale, 9 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1 * scale;
        for (let i = 0; i < 8; i++) {
          const fx = bx + (i - 3.5) * 2.5 * scale;
          ctx.beginPath();
          ctx.moveTo(fx, by + 2 * scale);
          ctx.lineTo(fx + Math.sin(i) * scale, by + 8 * scale);
          ctx.stroke();
        }
        ...
        ctx.beginPath();
        ctx.moveTo(bx, by - 5 * scale);
        ctx.quadraticCurveTo(bx + 2 * scale, by + 2 * scale, bx - 1 * scale, by + 8 * scale);
        ctx.stroke();
        ctx.lineCap = "butt";
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + dir * 3 * scale, by - 6 * scale);
          ctx.quadraticCurveTo(bx + dir * 8 * scale, by - 2 * scale, bx + dir * 6 * scale, by + 4 * scale);
          ctx.stroke();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 3 * scale, by - 10 * scale, 0.8 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.globalAlpha = 0.5;
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale, 8 * scale, 4 * scale, 0, Math.PI, 0);
        ctx.fill();
        ctx.globalAlpha = 1.0;
        ...
        for (let i = 0; i < 4; i++) {
          const lx = bx + (i - 1.5) * 5 * scale;
          ctx.beginPath();
          ctx.fillRect(lx - 2 * scale, by + 7 * scale, 4 * scale, 6 * scale);
        }
        break;
      }
      ...
      case "lava_golem": {
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by, 9 * scale, 11 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const crack of cracks) {
          ...
          for (let i = 0; i < crack.length; i++) {
            const [px, py] = crack[i];
            if (i === 0) ctx.moveTo(bx + px * scale, by + py * scale);
            else ctx.lineTo(bx + px * scale, by + py * scale);
          }
          ctx.stroke();
        }
        ctx.shadowBlur = 0;
        ...
        // Glowing core pulse
        const corePulse = 0.4 + Math.sin(t * 2) * 0.3;
        ctx.fillStyle = `rgba(255, 100, 0, ${corePulse})`;
        ctx.beginPath();
        ctx.ellipse(bx, by - 2 * scale, 3 * scale, 4 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#2a1200";
        ctx.beginPath();
        ctx.arc(bx, by - 11 * scale, 5 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + dir * 2 * scale, by - 11 * scale, 1.2 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.shadowBlur = 0;
        ...
        // Ember particles
        for (let i = 0; i < 5; i++) {
          const ex = bx + Math.sin(t * 2 + i * 1.3) * 8 * scale;
          const ey = by - 5 * scale - ((t * 10 + i * 8) % (12 * scale));
          ctx.fillStyle = `rgba(255, ${100 + i * 30}, 0, ${0.7 - i * 0.1})`;
          ctx.beginPath();
          ctx.arc(ex, ey, (0.6 + Math.sin(t * 4 + i) * 0.3) * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.ellipse(bx + dir * 10 * scale, by - 2 * scale, 3 * scale, 5 * scale, dir * 0.3, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      }
      ...
      case "volcanic_drake": {
        const bx = cx;
        const by = cy + bounce;
        const wingFlap = Math.sin(t * 3) * 0.25;

        // Body
        ctx.fillStyle = "#cc3300";
        ctx.beginPath();
        ctx.ellipse(bx, by, 7 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Fire wings
        for (const dir of [-1, 1]) {
          const wingGrad = ctx.createLinearGradient(bx, by, bx + dir * 14 * scale, by - 6 * scale);
          wingGrad.addColorStop(0, "#cc3300");
          wingGrad.addColorStop(0.5, "#ff6600");
          wingGrad.addColorStop(1, "#ffaa00");
          ctx.fillStyle = wingGrad;
          ctx.beginPath();
          ctx.moveTo(bx + dir * 2 * scale, by - 1 * scale);
          ctx.quadraticCurveTo(
            bx + dir * 8 * scale, by - (10 + wingFlap * 10) * scale,
            bx + dir * 14 * scale, by - (5 + wingFlap * 8) * scale
          );
          ctx.lineTo(bx + dir * 10 * scale, by + 2 * scale);
          ctx.closePath();
          ctx.fill();
        }
        ...
        ctx.lineWidth = 3 * scale;
        ctx.beginPath();
        ctx.moveTo(bx, by - 4 * scale);
        ctx.quadraticCurveTo(bx + 3 * scale, by - 8 * scale, bx + 2 * scale, by - 12 * scale);
        ctx.stroke();

        // Head
        ctx.fillStyle = "#cc3300";
        ctx.beginPath();
        ctx.ellipse(bx + 2 * scale, by - 13 * scale, 3 * scale, 2.5 * scale, -0.2, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.moveTo(bx + 2 * scale + dir * 2 * scale, by - 14 * scale);
          ctx.lineTo(bx + 2 * scale + dir * 3.5 * scale, by - 18 * scale);
          ctx.stroke();
        }
        ...
        ctx.shadowBlur = 4 * scale;
        ctx.beginPath();
        ctx.arc(bx + 3 * scale, by - 13.5 * scale, 0.8 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.shadowBlur = 0;
        ...
        ctx.lineWidth = 2 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 6 * scale, by + 3 * scale);
        ctx.quadraticCurveTo(bx - 12 * scale, by + 6 * scale, bx - 14 * scale, by + 3 * scale);
        ctx.stroke();
        // Tail flame
        ctx.fillStyle = "#ff8800";
        ctx.beginPath();
        const tailFlame = Math.sin(t * 5) * 1.5 * scale;
        ctx.moveTo(bx - 14 * scale, by + 3 * scale);
        ctx.lineTo(bx - 16 * scale + tailFlame, by + 1 * scale);
        ctx.lineTo(bx - 15 * scale, by + 4 * scale);
        ctx.closePath();
        ctx.fill();
        break;
      }
      ...
      case "salamander": {
        ...
        // Sleek body
        ctx.fillStyle = salamanderColor;
        ctx.beginPath();
        ctx.ellipse(bx, by, 8 * scale, 3.5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.5 * scale;
        ctx.beginPath();
        ctx.moveTo(bx - 7 * scale, by);
        for (let i = 0; i < 6; i++) {
          const sx = bx - 7 * scale + i * 2.5 * scale;
          const sy = by - 1 * scale + Math.sin(t * 4 + i) * 0.5 * scale;
          ctx.lineTo(sx, sy);
        }
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.ellipse(bx, by + 1 * scale, 6 * scale, 2 * scale, 0, 0, Math.PI * 2);
        ctx.fill();

        // Head
        ctx.fillStyle = "#ff5500";
        ctx.beginPath();
        ctx.ellipse(bx + 8 * scale, by - 1 * scale, 3 * scale, 2.5 * scale, -0.15, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 9 * scale, by - 1.5 * scale + dir * 1 * scale, 0.7 * scale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.fillStyle = "#000";
        for (const dir of [-1, 1]) {
          ctx.beginPath();
          ctx.arc(bx + 9 * scale, by - 1.5 * scale + dir * 1 * scale, 0.3 * scale, 0, Math.PI * 2);
          ctx.fill();
        }

        // 4 legs
        ctx.strokeStyle = salamanderColor;
        ctx.lineWidth = 1.5 * scale;
        const legPositions = [[-4, 1], [-1, 1], [2, 1], [5, 1]];
        for (const [lx, ly] of legPositions) {
          const side = lx < 1 ? -1 : 1;
          ctx.beginPath();
          ctx.moveTo(bx + lx * scale, by + ly * scale + 2 * scale);
          ctx.lineTo(bx + (lx + side * 2) * scale, by + 6 * scale);
          ctx.stroke();
        }
        ...
        ctx.globalAlpha = 0.6;
        for (let i = 0; i < 4; i++) {
          const fx = bx - (8 + i * 2.5) * scale;
          const fy = by + Math.sin(t * 5 + i) * 1 * scale;
          const fr = (2 - i * 0.4) * scale;
