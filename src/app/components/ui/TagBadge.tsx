...
export function TagIcon({ tag, size }: { tag: TowerTag; size: number }) {
  ...
  const Icon = TAG_ICON_MAP[def.icon];
  if (!Icon) return null;
  return <Icon size={size} className={def.textClass} />;
}

export function TagBadge({ tag, size = 8 }: { tag: TowerTag; size?: number }) {
  const def = TOWER_TAG_DEFS[tag];
  return (
    <span className={`inline-flex items-center gap-[2px] text-[7px] sm:text-[8px] font-bold uppercase tracking-wider px-1 sm:px-1.5 py-[1px] rounded border ${def.textClass} ${def.bgClass} ${def.borderClass}`}>
