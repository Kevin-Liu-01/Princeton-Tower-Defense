...
function computeElapsed(levelStartTime: number, totalPausedTime: number): number {
  if (levelStartTime <= 0) return 0;
  return Math.max(0, Math.floor((Date.now() - levelStartTime - totalPausedTime) / 1000));
}
...
export const GameTimer: React.FC<GameTimerProps> = ({
  ...
  useEffect(() => {
    if (!settings.ui.showGameTimer || levelStartTime <= 0) return;
    tick();
  ...
  if (!settings.ui.showGameTimer || levelStartTime <= 0) return null;
