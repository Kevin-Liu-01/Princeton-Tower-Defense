...
import { WORLD_LEVELS, DEV_LEVEL_IDS, type LevelNode } from "../world-map/worldMapData";
...
export function getCampaignLevels(isDevMode = false): LevelNode[] {
  return WORLD_LEVELS.filter(
    (level) => level.kind !== "sandbox" && (isDevMode || !DEV_LEVEL_IDS.has(level.id)),
  );
}

export function getRegionLevels(region: Region, isDevMode = false): LevelNode[] {
  return getCampaignLevels(isDevMode).filter((level) => level.region === region);
}
...
export function computeRegionProgress(
  ...
  const stars = levels.reduce((sum, level) => sum + (levelStars[level.id] || 0), 0);
  const maxStars = levels.length * 3;
  const completed = levels.filter((level) => (levelStars[level.id] || 0) > 0).length;
  const unlocked = levels.filter((level) => unlockedMaps.has(level.id)).length;
  const targetLevel =
    levels.find((level) => unlockedMaps.has(level.id) && (levelStars[level.id] || 0) < 3) ??
    levels[0] ??
    null;
