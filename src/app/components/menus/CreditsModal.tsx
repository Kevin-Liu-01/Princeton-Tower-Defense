        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="p-4 rounded-xl bg-amber-950/30 border border-amber-800/30">
            <p className="text-sm text-amber-200/80 leading-relaxed">
              ...
              game engine with zero WebGL, zero game framework dependencies, and
              zero external rendering libraries. Every pixel is drawn through{" "}
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-amber-200/70 leading-relaxed">
              Most browser games default to WebGL or a framework like
              Phaser/PixiJS. I wanted to see if a complex isometric tower
              defense &mdash; with hundreds of animated sprites, real-time
              projectile physics, atmospheric effects, and smooth 60fps gameplay
              &mdash; could be achieved using only the 2D canvas context. No
              shaders, no GPU abstraction layers &mdash; just{" "}
              <code className="text-amber-400/90 bg-amber-900/40 px-1.5 py-0.5 rounded text-xs">
                ctx.drawImage()
              </code>
              ,{" "}
...
function LinkButton({
  ...
  return (
    ...
    >
      <div className="text-amber-400/70 group-hover:text-amber-300 transition-colors">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-amber-200/80 group-hover:text-amber-200 transition-colors">
          {label}
        </div>
      ...
      <ExternalLink
        size={14}
        className="text-amber-200/20 group-hover:text-amber-200/50 transition-colors flex-shrink-0"
      />
    </a>
  );
}
