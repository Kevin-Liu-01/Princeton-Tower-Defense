...
export const drawWorldMapCanvas = ({
  ...
  const drawSphinx = (sx: number, syPct: number, scale: number) => {
    ...
    for (let c = 0; c < 5; c++) {
      ...
      ctx.moveTo(cx0, cy0);
      ctx.lineTo(
        cx0 + (seededRandom(sx + c * 13) - 0.5) * 8 * scale,
        cy0 + seededRandom(sx + c * 17) * 6 * scale,
      );
      ctx.stroke();
    }
  ...
  const drawPalmTree = (tx: number, tyPct: number, scale: number) => {
    ...
    frondAngles.forEach((angle, i) => {
      const sway =
        Math.sin(time * 1.2 + i * 1.1) * 0.06 +
        Math.sin(time * 2.5 + i * 0.7) * 0.03;
      ctx.save();
  ...
  const drawFrozenLake = (
    ...
    for (let b = 0; b < 8; b++) {
      ...
      const by =
        ly -
        width * heightRatio * 0.5 +
        seededRandom(lx + b * 17) * width * heightRatio;
    ...
    for (let sc = 0; sc < 4; sc++) {
      ...
      const scY =
        ly -
        width * heightRatio * 0.2 +
        seededRandom(lx + sc * 43) * width * heightRatio * 0.4;
    ...
    for (let s = 0; s < 4; s++) {
      ...
      const sy =
        ly -
        width * heightRatio * 0.3 +
        seededRandom(lx + s * 11) * width * heightRatio * 0.6;
      ctx.beginPath();
      ctx.ellipse(
        sx,
        sy,
  ...
  const drawVolcano = (
    ...
    for (let f = 0; f < 3; f++) {
      ...
      const flowMid1X =
        flowStartX + (flowEndX - flowStartX) * 0.3 + (seed - 0.5) * 12;
      const flowMid1Y = vy - heightPx * 0.65;
      const flowMid2X =
        flowStartX + (flowEndX - flowStartX) * 0.65 + (seed - 0.5) * 18;
  ...
  const drawLavaPool = (
    ...
    for (let cr = 0; cr < 5; cr++) {
      ...
      const crY =
        py -
        width * heightRatio * 0.3 +
        seededRandom(px + cr * 23) * width * heightRatio * 0.6;
    ...
    for (let b = 0; b < 6; b++) {
      ...
      const by =
        py -
        width * heightRatio * 0.3 +
        seededRandom(px + b + 10) * width * heightRatio * 0.6;
    ...
    ctx.ellipse(
      px + width * 0.15,
      py + width * heightRatio * 0.1,
      width * 0.2,
      width * heightRatio * 0.15,
      -0.2,
      0,
      Math.PI,
    );
  ...
  const drawLavaRiver = (points: number[][]) => {
    ...
    for (let r = 0; r < 5; r++) {
      ...
      const raftProgress =
        ((time * speed + r * 20 + points[0][0] * 0.1) % 100) / 100;
      const segIdx = Math.min(
        Math.floor(raftProgress * (points.length - 1)),
        points.length - 2,
      );
    ...
    for (let i = 1; i < points.length - 1; i++) {
      for (let s = 0; s < 2; s++) {
        ...
        const sx =
          points[i][0] + Math.sin(time * 1.5 + s + i) * 5 + (s === 0 ? -6 : 6);
