...
export const drawWorldMapCanvas = ({
  ...
  const drawKingdomCastle = (x: number, yPct: number, isEnemy: boolean) => {
    ...
    for (let row = 0; row < 5; row++) {
      const wy = y - 26 + row * 7;
      ctx.beginPath();
      ctx.moveTo(x - 20, wy + 1);
      ctx.lineTo(x + 20, wy + 1);
      ctx.stroke();
    }
    ...
    const drawRoundTower = (tx: number, isLeft: boolean) => {
      ...
      const drawTowerWindow = (wy: number) => {
        ...
        wGlowGrad.addColorStop(
          0,
          `rgba(${isEnemy ? "255,100,30" : "255,200,80"},${wGlow * 0.35})`,
        );
        wGlowGrad.addColorStop(
          1,
          `rgba(${isEnemy ? "255,60,10" : "255,150,40"},0)`,
        );
    ...
    for (let row = 0; row < 15; row++) {
      const ky = y - 88 + row * 6;
      ctx.beginPath();
      ctx.moveTo(x - keepW, ky);
      ctx.lineTo(x + keepW, ky);
      ctx.stroke();
    ...
    const keepRoofGrad = ctx.createLinearGradient(
      x - keepW - 4,
      y - 115,
      x + keepW + 4,
      y - 93,
    ...
    const drawKeepWindow = (wx: number, wy: number, ww: number, wh: number) => {
      ...
      haloGrad.addColorStop(
        0,
        `rgba(${isEnemy ? "255,80,20" : "255,200,80"},${wGlow * 0.3})`,
      );
      haloGrad.addColorStop(
        1,
        `rgba(${isEnemy ? "200,40,10" : "200,150,50"},0)`,
      );
      ctx.fillStyle = haloGrad;
      ctx.beginPath();
    ...
    for (let ch = 0; ch < 2; ch++) {
      ...
      for (let s = 0; s < 4; s++) {
        ...
        const sx =
          chx + Math.sin(time * 1.2 + s * 1.5 + ch) * (3 + sAge * 0.15);
